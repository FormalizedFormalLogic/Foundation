...
lemma log_exists_unique (y : M) : ∃! x, (y = 0 → x = 0) ∧ (0 < y → x < y ∧ ∃ y' ≤ y, Exponential x y' ∧ y < 2 * y') := by
  by_cases hy : y = 0
  · rcases hy; simp
...
lemma log_pos {y : M} (pos : 0 < y) : ∃ y' ≤ y, Exponential (log y) y' ∧ y < 2 * y' :=
...
instance log_definable : 𝚺₀-Function₁ (log : M → M) := Defined.to_definable _ log_defined

instance : Bounded₁ (log : M → M) := ⟨#0, λ _ ↦ by simp⟩
...
lemma Exponential.log_eq_of_exp {x y : M} (H : Exponential x y) : log y = x :=
  log_eq_of_pos H.range_pos H (by { rfl }) (lt_mul_of_pos_of_one_lt_left H.range_pos one_lt_two)

lemma exponential_of_pow2 {p : M} (pp : Pow2 p) : Exponential (log p) p := by
...
instance length_definable : 𝚺₀-Function₁ (‖·‖ : M → M) := Defined.to_definable _ length_defined

instance : Bounded₁ (‖·‖ : M → M) := ⟨#0, λ _ ↦ by simp⟩

@[simp] lemma length_one : ‖(1 : M)‖ = 1 := by simp [length_eq_binaryLength]

lemma Exponential.length_eq {x y : M} (H : Exponential x y) : ‖y‖ = x + 1 := by
...
lemma exponential_log_le_self {a b : M} (pos : 0 < a) (h : Exponential (log a) b) : b ≤ a := by
  rcases log_pos pos with ⟨_, _, H, _⟩; rcases H.uniq h
  assumption

lemma lt_exponential_log_self {a b : M} (h : Exponential (log a) b) : a < 2 * b := by
  rcases zero_le a with (rfl | pos)
  · simp at h; simp [h]
  rcases log_pos pos with ⟨_, _, H, _⟩; rcases H.uniq h
  assumption

lemma lt_exp_len_self {a b : M} (h : Exponential ‖a‖ b) : a < b := by
  rcases zero_le a with (rfl | pos)
  · simp at h; simp [h]
...
lemma le_iff_le_log_of_exp {x y a : M} (H : Exponential x y) (pos : 0 < a) : y ≤ a ↔ x ≤ log a :=
...
lemma le_iff_lt_length_of_exp {x y a : M} (H : Exponential x y) : y ≤ a ↔ x < ‖a‖ := by
  rcases zero_le a with (rfl | pos)
  · simp; exact pos_iff_ne_zero.mp H.range_pos
  simp [le_iff_le_log_of_exp H pos, length_of_pos pos, ←le_iff_lt_succ]

lemma Exponential.lt_iff_log_lt {x y a : M} (H : Exponential x y) (pos : 0 < a) : a < y ↔ log a < x :=
  not_iff_not.mp (by simpa using le_iff_le_log_of_exp H pos)

lemma Exponential.lt_iff_len_le {x y a : M} (H : Exponential x y) : a < y ↔ ‖a‖ ≤ x := not_iff_not.mp (by simpa using le_iff_lt_length_of_exp H)

lemma Exponential.le_of_lt_length {x y a : M} (H : Exponential x y) : x < ‖a‖ → y ≤ a := fun h ↦
  (le_iff_lt_length_of_exp H).mpr h

lemma Exponential.le_log {x y : M} (H : Exponential x y) : x ≤ log y := (le_iff_le_log_of_exp H H.range_pos).mp (by rfl)

lemma Exponential.lt_length {x y : M} (H : Exponential x y) : x < ‖y‖ := (le_iff_lt_length_of_exp H).mp (by rfl)

lemma lt_exponential_length {a b : M} (h : Exponential ‖a‖ b) : a < b := by
  rcases zero_le a with (rfl | pos)
  · simp at h; simp [h]
  simp [length_of_pos pos] at h
  rcases Exponential.exponential_succ.mp h with ⟨b, rfl, H⟩
...
lemma brange_exists_unique (a : M) : ∀ x < ‖a‖, ∃! y, Exponential x y := by
...
lemma bexp_exists_unique (a x : M) : ∃! y, (x < ‖a‖ → Exponential x y) ∧ (‖a‖ ≤ x → y = 0) := by
  by_cases hx : x < ‖a‖
...
lemma exp_bexp_of_lt {a x : M} (h : x < ‖a‖) : Exponential x (bexp a x) :=
...
@[simp] lemma exp_bexp_of_lt_iff {a x : M} : Exponential x (bexp a x) ↔ x < ‖a‖ :=
...
lemma bexp_graph {y a x : M} : y = bexp a x ↔ ∃ l ≤ a, l = ‖a‖ ∧ (x < l → Exponential x y) ∧ (l ≤ x → y = 0) :=
...
instance bexp_definable : 𝚺₀-Function₂ (bexp : M → M → M) := Defined.to_definable _ bexp_defined

instance : Bounded₂ (bexp : M → M → M) := ⟨#0, λ _ ↦ by simp⟩
...
lemma bexp_eq_of_exp {a x : M} (h : x < ‖a‖) (H : Exponential x y) : bexp a x = y := (exp_bexp_of_lt h).uniq H

lemma log_bexp {a x : M} (h : x < ‖a‖) : log (bexp a x) = x := Exponential.log_eq_of_exp (exp_bexp_of_lt h)
...
instance fbit_definable : 𝚺₀-Function₂ (fbit : M → M → M) := Defined.to_definable _ fbit_defined

instance : Bounded₂ (fbit : M → M → M) := ⟨‘1’, λ _ ↦ by simp⟩
