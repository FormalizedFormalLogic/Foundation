...
- Do this work in a fresh worktree, at whatever location the local (untracked) agent instructions for this checkout specify. Never work directly on `master`.
- If those instructions define a place for per-worktree progress notes, add an entry there and keep it updated as you go.
...
- Normalize to `warning: <file>:<line>:<col>` locations and save the list for later comparison:
  ...
  ```
- If the build times out, continue it in the background and wait for it to finish. One full build is enough — don't run it repeatedly and pile up duplicate output.
...
- **Every `error:` is a breaking change from the new version and must be fixed.** Common causes seen in practice:
  ...
  - An existing `by simp` becomes `simp made no progress` because mathlib's simp set changed — check the actual goal with the `lean-lsp` MCP server (`lean_goal`, `lean_multi_attempt`) and replace it with the right lemma/tactic.
...
- Warnings that already existed before the bump are out of scope for this PR. Still, record which files/warnings were left untouched as pre-existing, so they remain a tracked follow-up.
