...
lemma not_goedel_unprovable [ℕ ⊧ₘ* T] : T ⊬ ∼↑𝗚 := fun h ↦ by
  ...
  have : T ⊢!. □𝗚 := Entailment.contra_CN!' (K!_right goedel_iff_unprovable_goedel) ⨀ (by simpa [provable₀_iff] using h)
...
lemma consistent_iff_goedel : T ⊢! ↑𝗖𝗼𝗻 ⭤ ↑𝗚 := by
  apply E!_intro
  · have bew_G : [∼𝗚] ⊢[T.alt]! □𝗚 := deductInv'! <| contra_CN!' <| K!_right goedel_iff_unprovable_goedel
    ...
    simpa [provable₀_iff] using contra_CN!' (deduct'! this)
  · have : [□⊥] ⊢[T.alt]! □𝗚 := by
      have : T ⊢!. □(⊥ ➝ 𝗚) := provableₐ_D1 efq!
      exact provableₐ_D2_context (of'! this) (by simp)
    have : [□⊥] ⊢[T.alt]! ∼𝗚 :=
      of'! (contra_CN! <| K!_left <| goedel_iff_unprovable_goedel) ⨀ this
    simpa [provable₀_iff] using  contra_CN! (deduct'! this)
