...
def Graph₄ (f : α → β → γ → δ → σ) : σ → α → β → γ → δ → Prop :=
  fun y x₁ x₂ x₃ x₄ ↦ y = f x₁ x₂ x₃ x₄

def Graph₅ (f : α → β → γ → δ → ε → σ) : σ → α → β → γ → δ → ε → Prop :=
...
lemma Graph₂.iff_left (f : α → β → σ) {y x₁ x₂} : f x₁ x₂ = y ↔ Graph₂ f y x₁ x₂ := by
  simp [Graph₂, eq_comm]

lemma Graph₂.iff_right (f : α → β → σ) {y x₁ x₂} : y = f x₁ x₂ ↔ Graph₂ f y x₁ x₂ := by
  simp [Graph₂]

lemma Graph₃.eq {f : α → β → γ → σ} {y x₁ x₂ x₃} (h : Graph₃ f y x₁ x₂ x₃) :
    f x₁ x₂ x₃ = y := h.symm

lemma Graph₃.iff_left (f : α → β → γ → σ) {y x₁ x₂ x₃} :
    f x₁ x₂ x₃ = y ↔ Graph₃ f y x₁ x₂ x₃ := by
  simp [Graph₃, eq_comm]

lemma Graph₃.iff_right (f : α → β → γ → σ) {y x₁ x₂ x₃} :
