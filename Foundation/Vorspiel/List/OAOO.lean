...
/-!
# One and only one of a list of propositions

`l.OAOO` states that one and only one of the propositions in `l` holds, in the style of
`List.TFAE`. The tactic `oaoo_split` reduces a goal `[p₁, …, pₙ].OAOO` to the disjunction
...
/-- One and only one of the propositions in `l` holds. -/
def OAOO (l : List Prop) : Prop := (∃ p ∈ l, p) ∧ l.Pairwise fun p q ↦ ¬(p ∧ q)

variable {l : List Prop} {p : Prop}

lemma OAOO.intro (h₁ : ∃ p ∈ l, p) (h₂ : l.Pairwise fun p q ↦ ¬(p ∧ q)) : l.OAOO :=
  ⟨h₁, h₂⟩

lemma OAOO.exists (h : l.OAOO) : ∃ p ∈ l, p := h.1

@[simp] lemma not_oaoo_nil : ¬[].OAOO := by simp [OAOO]

lemma oaoo_cons : (p :: l).OAOO ↔ p ∧ (∀ q ∈ l, ¬q) ∨ ¬p ∧ l.OAOO := by
  by_cases hp : p;
  · suffices (p :: l).OAOO ↔ ∀ q ∈ l, ¬q by simpa [hp];
    constructor;
  ...
  · rw [OAOO, OAOO, exists_mem_cons_iff, pairwise_cons];
    simp [hp]

lemma OAOO.not_getElem (h : l.OAOO) {i j : ℕ} (hi : i < l.length) (hj : j < l.length)
...
lemma oaoo_iff_existsUnique : l.OAOO ↔ ∃! i : Fin l.length, l[i] := by
  constructor;
  · intro h;
...
/-- `oaoo_split` reduces a goal `[p₁, …, pₙ].OAOO` to the disjunction `p₁ ∨ ⋯ ∨ pₙ` and
the exclusivity `¬(pᵢ ∧ pⱼ)` of each pair `i < j`, in this order. -/
macro "oaoo_split" : tactic => `(tactic| (
  apply List.OAOO.intro
  on_goal 2 =>
