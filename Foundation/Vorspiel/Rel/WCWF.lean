...
lemma WeaklyConverseWellFounded.has_max [IsWeaklyConverseWellFounded α r] (s : Set α)
  ...
  obtain ⟨m, hm, h⟩ := ConverseWellFounded.iff_has_max.mp (IsWeaklyConverseWellFounded.wcwf (rel := r)) s hs;
...
instance [IsTrans α r] [Std.Antisymm r] : IsTrans α r.IrreflGen where
  trans a b c hab hbc :=
    ⟨IsTrans.trans a b c hab.1 hbc.1, by rintro rfl; exact hab.2 (Std.Antisymm.antisymm a b hab.1 hbc.1)⟩
