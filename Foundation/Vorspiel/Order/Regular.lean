...
@[simp] theorem coe_sSup (s : Set (Regular α)) : (↑(sSup s) : α) = (sSup ((↑) '' s))ᶜᶜ := by
  simp [sSup, sSup_image]

@[simp] theorem coe_iSup (a : ι → Regular α) : (↑(⨆ i, a i) : α) = (⨆ i, (a i : α))ᶜᶜ := calc
  (↑(⨆ i, a i) : α) = (↑(sSup (Set.range a)) : α) := by rfl
                  _ = (⨆ i, (a i : α))ᶜᶜ          := by rw [coe_sSup]; congr; grind

@[simp] theorem coe_sInf (s : Set (Regular α)) : (↑(sInf s) : α) = sInf ((↑) '' s) := by
  simp [sInf, sInf_image]; rfl

@[simp] theorem coe_iInf (a : ι → Regular α) : (↑(⨅ i, a i) : α) = (⨅ i, (a i : α)) := calc
