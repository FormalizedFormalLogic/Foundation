...
@[simp] lemma mem_vector_mOfFn {α : Type*} : ∀ {n : ℕ} {w : List.Vector α n} {v : Fin n →. α},
...
lemma unit_dom_iff (x : Part Unit) : x.Dom ↔ () ∈ x := by
  simp [dom_iff_mem, show ∀ x : Unit, x = () by intro x; rfl]
