...
theorem kripke_sound' : Modal.K _ ⊢ φ → (∀ κ, [Nonempty κ] → ∀ M : KripkeModel κ α, M ⊧ φ)
  := fun h κ [Nonempty κ] M ↦ kripke_sound h κ M.frame M.val
...
theorem kripke_complete : (∀ κ : Type u, [Nonempty κ] → ∀ M : KripkeModel κ α, M ⊧ φ) → Modal.K _ ⊢ φ
  := fun h ↦ canonicalKripkeModel.iff_valid_provable.mp $ h _ _

theorem iff_provable_valid_all_kripkeModel : Modal.K _ ⊢ φ ↔ (∀ κ : Type u, [Nonempty κ] → ∀ M : KripkeModel κ α, M ⊧ φ) := by
  constructor;
  . apply kripke_sound';
