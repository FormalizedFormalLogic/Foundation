...
class FilterOf (FM : Model) (M : Model) (T : FormulaSet ℕ) [T.SubformulaClosed] : Prop where
  ...
  def_box : ∀ {Qx Qy : FM.World}, Qx ≺ Qy → Quotient.lift₂ (λ x y => ∀ φ, □φ ∈ T → (x ⊧ □φ → y ⊧ φ)) (by
    ...
    constructor;
    . intro h φ hp sp₂; exact hy φ (by sorry) |>.mp $ h φ hp $ hx (□φ) hp |>.mpr sp₂;
    . intro h φ hp sp₁; exact hy φ (by sorry) |>.mpr $ h φ hp $ hx (□φ) hp |>.mp sp₁;
...
theorem filteration {x : M.World} {φ : Formula ℕ} (hs : φ ∈ T) : x ⊧ φ ↔ (cast (filterOf.def_world.symm) ⟦x⟧) ⊧ φ := by
  ...
  | himp φ ψ ihp ihq =>
    constructor;
    . rintro hxy hp;
      exact ihq (by sorry) |>.mp $ hxy (ihp (FormulaSet.SubformulaClosed.mem_imp₁ hs) |>.mpr hp);
    . rintro hxy hp;
      exact ihq (by sorry) |>.mpr $ hxy (ihp (FormulaSet.SubformulaClosed.mem_imp₁ hs) |>.mp hp);
  | _ => trivial
