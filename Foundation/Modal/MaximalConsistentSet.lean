...
lemma def_consistent [Entailment.Minimal 𝓢] : Consistent 𝓢 T ↔ ∀ Γ : FormulaFinset α, ((↑Γ : Set _) ⊆ T) → (↑Γ : FormulaSet _) *⊬[𝓢] ⊥ := by
  constructor;
  . intro h Γ hΓ;
...
lemma def_inconsistent [Entailment.Minimal 𝓢] : Inconsistent 𝓢 T ↔ ∃ (Γ : FormulaFinset α), ((↑Γ : Set _) ⊆ T) ∧ (↑Γ : FormulaSet _) *⊢[𝓢] ⊥ := by
  unfold Inconsistent;
...
lemma iff_insert_inconsistent : Inconsistent 𝓢 (insert φ T) ↔ ∃ Γ : FormulaFinset α, ((↑Γ : Set _) ⊆ T) ∧ (↑Γ : FormulaSet _) *⊢[𝓢] φ 🡒 ⊥ := by
  unfold Inconsistent;
  apply not_iff_not.mp;
  push Not;
...
open Classical in
lemma intro_union_consistent(h : ∀ {Γ₁ Γ₂ : FormulaFinset α}, ((↑Γ₁ : Set _) ⊆ T₁) → ((↑Γ₂ : Set _) ⊆ T₂) → (↑(Γ₁ ∪ Γ₂) : FormulaSet _) *⊬[𝓢] ⊥)
  : Consistent 𝓢 (T₁ ∪ T₂) := by
  apply def_consistent.mpr;
  intro Δ hΔ;
  let Δ₁ := (Δ.filter (· ∈ T₁));
  ...
  · have hΔ₁ : (↑Δ₁ : Set (Formula α)) ⊆ T₁ := by
      intro x hx; simp only [Δ₁, Finset.coe_filter, Set.mem_setOf_eq] at hx; exact hx.2
    have hΔ₂ : (↑Δ₂ : Set (Formula α)) ⊆ T₂ := by
      intro x hx; simp only [Δ₂, Finset.coe_filter, Set.mem_setOf_eq] at hx; exact hx.2
    have key := h hΔ₁ hΔ₂
    rw [Finset.coe_union] at key
    exact key
  . grind;
...
lemma exists_consistent_maximal_of_consistent (T_consis : Consistent 𝓢 T)
  ...
  obtain ⟨Z, h₁, ⟨h₂, h₃⟩⟩ := zorn_subset_nonempty { T : FormulaSet α | Consistent 𝓢 T} (by
    ...
    . apply def_consistent.mpr;
      intro Γ hΓ;
      by_contra hC;
      obtain ⟨U, hUc, hUs⟩ := Set.subset_mem_chain_of_finite c hnc chain (s := ↑Γ) (by simp) $ by
        intro φ hφ;
        apply hΓ hφ;
...
lemma iff_mem_imp : ((φ 🡒 ψ) ∈ Ω) ↔ (φ ∈ Ω) → (ψ ∈ Ω) := by
  ...
  . intro hpq hp;
    ...
    exact dpq ⨀ dp;
  . intro h;
    replace h : φ ∉ Ω.1 ∨ ψ ∈ Ω := or_iff_not_imp_left.mpr (by simp only [not_not]; exact h);
    cases h with
    | inl h =>
      apply membership_iff.mpr;
