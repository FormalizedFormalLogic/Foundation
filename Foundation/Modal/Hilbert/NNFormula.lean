...
lemma iff_neg {φ : NNFormula _} : Logic.K ⊢! ∼(φ.toFormula) ⭤ (∼φ).toFormula := by
  induction φ using NNFormula.rec' with
  | hNatom a => apply K!_intro <;> simp;
  | hAnd φ ψ ihφ ihψ =>
    apply K!_intro;
    . apply deduct'!;
      apply A!_replace $ ANN!_of_NK! $ show [∼(φ.toFormula ⋏ ψ.toFormula)] ⊢[Logic.K]! ∼(φ.toFormula ⋏ ψ.toFormula) by simp;
      . apply of'! $ K!_left ihφ;
      . apply of'! $ K!_left ihψ;
    . apply deduct'!;
      apply NK!_of_ANN!;
      apply A!_replace $ show [(∼φ).toFormula ⋎ (∼ψ).toFormula] ⊢[Logic.K]! (∼φ).toFormula ⋎ (∼ψ).toFormula by simp;
      . apply of'! $ K!_right ihφ;
      . apply of'! $ K!_right ihψ;
  | hOr φ ψ ihφ ihψ =>
    apply K!_intro;
    . apply deduct'!;
      apply K!_replace $ KNN!_of_NA! $ show [∼(φ.toFormula ⋎ ψ.toFormula)] ⊢[Logic.K]! (∼(φ.toFormula ⋎ ψ.toFormula)) by simp;
      . apply of'! $ K!_left ihφ;
      . apply of'! $ K!_left ihψ;
    . apply deduct'!;
      apply NA!_of_KNN!;
      apply K!_replace $ show [(∼φ).toFormula ⋏ (∼ψ).toFormula] ⊢[Logic.K]! (∼φ).toFormula ⋏ (∼ψ).toFormula by simp;
      . apply of'! $ K!_right ihφ;
      . apply of'! $ K!_right ihψ;
  | hBox φ ih =>
    apply K!_intro;
...
lemma exists_iff {φ} : ∃ ψ : NNFormula _, Logic.K ⊢! φ ⭤ ψ.toFormula := by
  induction φ with
  ...
  | himp φ ψ ihφ ihψ =>
    ...
    apply K!_intro;
    . apply deduct'!;
      apply A!_replace $ AN!_of_C! (show [φ ➝ ψ] ⊢[Logic.K]! φ ➝ ψ by simp;);
      . apply of'!;
...
lemma exists_of_provable {φ} (h : Logic.K ⊢! φ) : ∃ ψ : NNFormula _, Logic.K ⊢! ψ.toFormula := by
...
lemma exists_CNFPart_list {φ : NNFormula _} (φ_CNFP : φ.isModalCNFPart)
  : ∃ Γ : List { φ : NNFormula ℕ // φ.isPrebox ∨ φ.isPredia ∨ φ.degree = 0 }, Logic.K ⊢! φ.toFormula ⭤ ⋁(Γ.map (·.1)) := by
  induction φ using NNFormula.rec' with
...
lemma exists_CNF_list {φ : NNFormula _} (φ_CNF : φ.isModalCNF)
  : ∃ Γ : List { φ : NNFormula ℕ // φ.isModalCNFPart }, Logic.K ⊢! (φ.toFormula ⭤ ⋀(Γ.map (·.1))) := by
  induction φ using NNFormula.rec' with
  | hAtom a => use [⟨(NNFormula.atom a), by tauto⟩]; simp;
  | hNatom a => use [⟨(NNFormula.natom a), by tauto⟩]; simp;
  | hVerum => use []; simp;
...
theorem exists_CNF_DNF {φ : NNFormula _}
  : (∃ ψ : NNFormula _, ψ.isModalCNF ∧ Logic.K ⊢! φ.toFormula ⭤ ψ.toFormula) ∧
    (∃ ξ : NNFormula _, ξ.isModalDNF ∧ Logic.K ⊢! φ.toFormula ⭤ ξ.toFormula)
  := by
  induction φ using NNFormula.rec' with
...
lemma exists_DNF (φ : NNFormula _)
  : ∃ ψ : NNFormula _, ψ.isModalDNF ∧ Logic.K ⊢! φ.toFormula ⭤ ψ.toFormula
