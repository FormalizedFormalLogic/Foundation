...
lemma axm' {φ} (h : φ ∈ Ax) : Normal Ax ⊢ φ := by
  ...
  have h2 := axm (s := .id) h; rw [Formula.subst.def_id] at h2; exact h2;

@[grind <=] lemma axm! {φ} (s : Substitution _) (h : φ ∈ Ax) : Normal Ax ⊢ φ⟦s⟧ := by
  apply Logic.iff_provable.mpr;
  apply axm s h;

@[grind <=] lemma axm'! {φ} (h : φ ∈ Ax) : Normal Ax ⊢ φ := by
...
instance [Ax.HasP] : Entailment.HasAxiomP (Hilbert.Normal Ax) where
  P := by
    constructor;
    have h := Hilbert.Normal.axm (Ax := Ax) (s := .id) HasP.mem_P;
    simp at h;
    exact h
