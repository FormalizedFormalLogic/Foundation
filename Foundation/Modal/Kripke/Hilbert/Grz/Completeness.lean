...
lemma Context.multibox_2_in_context_to_box_finset [Entailment.HasAxiomFour 𝓢] {Γ : Finset F} (h : Γ.multibox 2 *⊢[𝓢]! φ) : Γ.box *⊢[𝓢]! φ := by
  apply FConj_DT.mp;
...
lemma Context.multibox_2_in_context_to_box [Entailment.HasAxiomFour 𝓢] {Γ : Set F} (h : Γ.multibox 2 *⊢[𝓢]! φ) : Γ.box *⊢[𝓢]! φ := by
...
lemma Context.boxbox_in_context_to_box [Entailment.HasAxiomFour 𝓢] {Γ : Set F} (h : Γ.box.box *⊢[𝓢]! φ) : Γ.box *⊢[𝓢]! φ := by
