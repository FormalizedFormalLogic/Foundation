...
instance [F.IsGeachConvergent ⟨0, 0, 1, 1⟩] : F.IsSerial where
  serial := by have h := IsGeachConvergent.gconv (F := F) (g := ⟨0, 0, 1, 1⟩); simp at h; exact h;
...
instance [F.IsGeachConvergent ⟨1, 1, 1, 1⟩] : F.IsPiecewiseStronglyConvergent where
  ps_convergent := by have h := IsGeachConvergent.gconv (g := ⟨1, 1, 1, 1⟩) (F := F); simp at h; exact h;
