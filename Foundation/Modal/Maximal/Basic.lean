...
lemma Hilbert.Normal.provable_of_classical_provable {Ax : Axiom ℕ} {φ : Propositional.Formula ℕ} : Propositional.Hilbert.Cl ⊢! φ → (Hilbert.Normal Ax ⊢! φ.toModalFormula) := by
  intro h;
...
lemma iff_provable_Cl : Modal.Triv ⊢! φ ↔ Propositional.Hilbert.Cl ⊢! φᵀ.toPropFormula := by
  constructor;
  . intro h;
...
protected lemma iff_provable_Cl : Modal.Ver ⊢! φ ↔ Propositional.Hilbert.Cl ⊢! φⱽ.toPropFormula := by
  constructor;
  . intro h;
    induction h using Hilbert.Normal.rec! with
    | axm s a =>
