...
lemma Logic.gS4_of_Int : Hilbert.Int ⊢! φ → Modal.S4 ⊢! φᵍ := by
...
lemma Logic.gGrz_of_Int : Hilbert.Int ⊢! φ → Modal.Grz ⊢! φᵍ := λ h ↦ WeakerThan.pbl $ gS4_of_Int h
...
lemma Logic.iff_provable_Cl_provable_dia_gS4 : 𝐂𝐥 ⊢! φ ↔ Modal.S4 ⊢! ◇φᵍ := by
  constructor;
  . intro h;
