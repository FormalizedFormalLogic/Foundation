...
lemma Logic.S.provable_TBB : 𝐒 over α ⊢ TBB n := by
  simpa [TBB] using S.axiomT;

lemma Logic.D.provable_TBB : 𝐃 over α ⊢ TBB n := by
  classical
  cases n with
  | zero => exact D.axiomP;
...
lemma provable_TBB : 𝐀 over α ⊢ TBB n := sumQuasiNormal.mem₂ ⟨n, trivial, rfl⟩

lemma neg_boxItr_bot : 𝐀 over α ⊢ ∼□^[n]⊥ := by
  induction n with
...
lemma GL_ssubset : 𝐆𝐋 over α ⊂ 𝐀 :=
  ⟨fun _ ↦ of_GL, fun h ↦
    GL.sound (pointModel fun _ ↦ True) (h (provable_TBB (n := 0))) 0 fun _ h ↦ h.elim⟩

lemma ssubset_D [Inhabited α] : 𝐀 over α ⊂ 𝐃 :=
