...
theorem sumQuasiNormal_eq_GLAlpha (h : ∀ A ∈ X, (trace A).Finite) :
    (𝐆𝐋 over α +ᴸ X.lift) = 𝐆𝐋α X.trace := by
...
theorem sumQuasiNormal_eq_GLBetaMinus (h : ∃ B ∈ X, (spectrum B).Finite) :
    ∃ hX : X.traceᶜ.Finite, (𝐆𝐋 over α +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX := by
  obtain ⟨B, hB, hfin⟩ := h;
...
theorem sumQuasiNormal_eq_GLAlpha_or_GLBetaMinus :
    ((∀ A ∈ X, (trace A).Finite) ∧ (𝐆𝐋 over α +ᴸ X.lift) = 𝐆𝐋α X.trace) ∨
    ∃ hX : X.traceᶜ.Finite, (𝐆𝐋 over α +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX := by
...
theorem sumQuasiNormal_classification :
    (X.Regular T ∧ (𝐆𝐋 over α +ᴸ X.lift) = 𝐆𝐋α X.trace) ∨
    (¬X.Regular T ∧ ∃ hX : X.traceᶜ.Finite, (𝐆𝐋 over α +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX) := by
