...
lemma Regular.of_imp [𝗜𝚺₁ ⪯ T] (h : A 🡒 B ∈ (𝐆𝐋 : Logic Empty)) (hA : A.Regular T) :
    B.Regular T := by
...
theorem sumQuasiNormal_classification :
    (X.Regular T ∧ ((𝐆𝐋 : Logic α) +ᴸ X.lift) = 𝐆𝐋α X.trace) ∨
    (¬X.Regular T ∧ ∃ hX : X.traceᶜ.Finite, ((𝐆𝐋 : Logic α) +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX) := by
