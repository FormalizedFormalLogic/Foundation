...
theorem sumQuasiNormal_eq_GLBetaMinus (h : ∃ B ∈ X, (spectrum B).Finite) :
    ∃ hX : X.traceᶜ.Finite, (𝐆𝐋@α +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX := by
  obtain ⟨B, hB, hfin⟩ := h;
...
theorem sumQuasiNormal_eq_GLAlpha_or_GLBetaMinus :
    ((∀ A ∈ X, (trace A).Finite) ∧ (𝐆𝐋@α +ᴸ X.lift) = 𝐆𝐋α X.trace) ∨
    ∃ hX : X.traceᶜ.Finite, (𝐆𝐋@α +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX := by
...
lemma Regular.of_imp [𝗜𝚺₁ ⪯ T] (h : A 🡒 B ∈ 𝐆𝐋@Empty) (hA : A.Regular T) :
    B.Regular T := by
...
theorem sumQuasiNormal_classification :
    (X.Regular T ∧ (𝐆𝐋@α +ᴸ X.lift) = 𝐆𝐋α X.trace) ∨
    (¬X.Regular T ∧ ∃ hX : X.traceᶜ.Finite, (𝐆𝐋@α +ᴸ X.lift) = 𝐆𝐋β⁻ X.trace hX) := by
