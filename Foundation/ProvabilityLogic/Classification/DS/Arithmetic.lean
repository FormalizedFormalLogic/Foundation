...
theorem S_subset_provabilityLogic
    ...
    (h : 𝐃 over α ⊂ T.provabilityLogicRelativeTo U) :
    𝐒 over α ⊆ T.provabilityLogicRelativeTo U := by
...
theorem not_D_ssubset_provabilityLogic_ssubset_S
    (hT : (T.provabilityLogicRelativeTo U : Logic α).trace = .univ) :
    ¬(𝐃 over α ⊂ T.provabilityLogicRelativeTo U ∧
      T.provabilityLogicRelativeTo U ⊂ 𝐒 over α) :=
