...
variable {L : Language} [L.ReferenceableBy L] {T₀ T : Theory L} [T₀ ⪯ T] {𝔅 : Provability T₀ T} [𝔅.HBL]
         {κ α : Type*} [Nonempty κ] {A : ProvabilityLogic.Formula α}

structure Provability.SolovaySentences (𝔅 : Provability T₀ T) (M : RootedModel κ α) [Fintype M.World] where
...
noncomputable def standardSolovaySentences (T : ArithmeticTheory) [T.Δ₁] [𝗜𝚺₁ ⪯ T]
    (M : RootedModel κ α) [Fintype M.World] [M.IsGL] : T.standardProvability.SolovaySentences M where
  σ := T.solovay M
