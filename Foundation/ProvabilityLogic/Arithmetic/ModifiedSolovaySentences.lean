...
theorem reflection :
  ...
  classical
  have h₁ {z : M.World} (hr : z ≠ M.root) (hu : z ≠ M.u) : T₀ ⊢ S (some z) 🡒 𝔅^[z.rank + 1] ⊥ := by
    induction z using WellFounded.induction IsConverseWellFounded.cwf (r := flip M.Rel) with
    | h z ih =>
      suffices T₀ ⊢ (⩖ j ∈ { j : M.extendRoot.World | some z ≺ j }, S j) 🡒 𝔅^[z.rank] ⊥ by
  ...
  have h₂ : T₀ ⊢ (⩖ j, S j) 🡒 ∼𝔅^[M.height] ⊥ 🡒 𝔅 σ 🡒 ∼σ 🡒 S (some M.root) := by
    ...
    · cl_prover [S.SC7];
    cl_prover [C_trans (h₁ hr hu) <| 𝔅.provable_boxItr_bot_mono <| rank_lt_height <|
      M.root_rel z hr];
  have h₃ : T₀ ⊢ 𝔅.conItr M.height 🡒 𝔅 σ 🡒 ∼σ 🡒 S (some M.root) := h₂ ⨀ S.SC5;
  cl_prover [h₃, S.mainlemma_neg (Option.some_ne_none M.root) Formula.mem_subfmls_self <|
...
lemma rew_chainAux (ε : List M.extendRoot.World) :
  ...
  |         [_] => simp [chainAux];
  | _ :: x :: ε =>
    simp [chainAux, moveAux, rew_chainAux (x :: ε), Finset.map_conj', Function.comp_def,
...
lemma Move.exists_toll (h : Move T M σ θ V x y) : ∃ w, Toll T M σ θ V y w :=
  let ⟨w, hw, _⟩ := h.2.2 y h.1 le_rfl; ⟨w, hw⟩
...
lemma Move.unique {y₁ y₂ : M.extendRoot.World} (h₁ : Move T M σ θ V x y₁)
  ...
  exfalso;
  obtain ⟨w₁, hw₁, hy₂⟩ := h₁.2.2 y₂ h₂.1 hlt.le;
  obtain ⟨w₂, hw₂, hy₁⟩ := h₂.2.1 y₁ h₁.1 hlt;
  rcases lt_or_ge w₂ w₁ with hw | hw;
  · exact hy₂ w₂ hw hw₂;
  · exact hy₁ w₁ hw hw₁;
...
lemma ModifiedSolovay.provable_disjunction {V : Type} [ORingStructure V] [V↓[ℒₒᵣ] ⊧* 𝗜𝚺₁]
  ...
  have h₁ : T.internalize V ⊢ ⌜H T M σ θ x 🡒 T.modifiedSolovay M σ θ x ⋎
    ...
    internal_provable_of_outer_provable <| WeakerThan.pbl (𝓢 := 𝗜𝚺₁) <|
      complete _ _ fun (V : Type) _ _ ↦ by simpa [models_iff] using! Reach.disjunction (hθσ V);
  ...
  have h₃ : 𝗜𝚺₁ ⊢ T.modifiedSolovay M σ θ none 🡒
    ...
    complete _ _ fun (V : Type) _ _ ↦ by
      simpa [models_iff, standardProvability_def] using! fun h ↦
        ModifiedSolovay.consistent (x := none) (y := some M.root) trivial
          ((Option.some_injective _).ne (ne_of_irrefl M.root_rel_u)) h;
  cl_prover [h₂, h₃];
...
def standardModifiedSolovaySentences
  ...
  { Λ := T.modifiedSolovay M σ hex.choose
    ...
    SC4 := complete _ _ fun (V : Type) _ _ ↦ by
      simpa [models_iff, standardProvability_def] using! ModifiedSolovay.provable_disjunction hθσ
