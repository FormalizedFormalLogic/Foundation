...
variable {T₀ T : FirstOrder.Theory ℒₒᵣ} [T₀ ⪯ T] [Diagonalization T₀]
         {𝔅 : Provability T₀ T} [𝔅.HBL] [ℕ↓[_] ⊧* T] [𝔅.SoundOn ℕ]
         {A B : Formula ℕ}

theorem S.arithmetical_soundness (h : Modal.S ⊢ A) (f : Realization 𝔅) : ℕ↓[_] ⊧ f A := by
  ...
  | mdp ihAB ihA =>
    simp only [Realization.interpret, LO.Semantics.Imp.models_imply] at ihAB;
    apply ihAB ihA;
