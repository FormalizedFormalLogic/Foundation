...
open FFL.ProvabilityLogic FFL.ProvabilityLogic.Kripke FFL.ProvabilityLogic.Kripke.Model
  FFL.ProvabilityLogic.Kripke.Model.World
...
variable {κ : Type*} [Nonempty κ]
         {α : Type*}
         {A B : ProvabilityLogic.Formula α}
         {M : RootedModel κ α}
...
noncomputable def realization : Realization α L :=
  ⟨fun a ↦ ⩖ i ∈ { i : M.World | i ⊩[M.toModel] (.atom a) }, S.σ i⟩

private lemma mainlemma_aux (hri : M.root ≠ i)
  : (i ⊩[M.toModel] A → T₀ ⊢ S.σ i 🡒 A.interpret S.realization 𝔅) ∧
  (i ⊮[M.toModel] A → T₀ ⊢ S.σ i 🡒 ∼(A.interpret S.realization 𝔅)) := by
...
theorem mainlemma (hri : M.root ≠ i) :
  i ⊩[M.toModel] A → T₀ ⊢ S.σ i 🡒 A.interpret S.realization 𝔅 := (mainlemma_aux hri).1

theorem mainlemma_neg (hri : M.root ≠ i) :
  i ⊮[M.toModel] A → T₀ ⊢ S.σ i 🡒 ∼(A.interpret S.realization 𝔅) := (mainlemma_aux hri).2
...
lemma theory_height (hSound : ∀ {σ}, T₀ ⊢ 𝔅 σ → T ⊢ σ) (h : M.root ⊩[M.toModel] ◇(∼A))
  ...
  obtain ⟨i, hi, hiA⟩ : ∃ i : M.World, M.root ≺ i ∧ i ⊮[M.toModel] A := by
...
def Θ (i : M.World) : Prop := ∃ ε : List M.World, ε.ChainI (fun x y ↦ y ≺ x) i M.root ∧ ΘChain T M V ε
...
lemma Solovay.refute (ne : M.root ≠ i) : T.Solovay M V i → Provable T (⌜∼T.solovay M i⌝ : V) := by
  intro h
...
variable {κ : Type*} [Nonempty κ] {α : Type*} {A : ProvabilityLogic.Formula α}
...
theorem unprovable_realization_exists
  (T : FirstOrder.ArithmeticTheory) [T.Δ₁] [𝗜𝚺₁ ⪯ T]
  (M : RootedModel κ α) [Fintype M.World] [M.IsGL]
  (hA : M.root ⊮[M.toModel] A) (h : M.height < T.height)
  : ∃ f : Realization α ℒₒᵣ, T ⊬ f T A := by
  let S := FFL.FirstOrder.Theory.standardProvability.solovaySentences (M := M.extendRoot) (T := T);
  use S.realization;
  contrapose! h;
  apply Order.le_of_lt_add_one;
  calc
    T.height < M.extendRoot.height := S.theory_height (T.standardProvability.syntactical_sound ℕ) (A := A) ?_ h
    _        = _                       := by
