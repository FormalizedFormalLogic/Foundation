...
lemma setShift_quote [L.LORDefinable] (Γ : Finset (SyntacticFormula L)) : setShift L (⌜Γ⌝ : V) = ⌜Finset.image Rewriting.shift Γ⌝ := by
  apply mem_ext
...
noncomputable def codeIn [L.DecidableEq] {Γ : Finset (SyntacticFormula L)} : T ⊢₂ Γ → V
  ...
  | and (Δ := Δ) _ (φ := φ) (ψ := ψ) bp bq => Metamath.andIntro ⌜Δ⌝ ⌜φ⌝ ⌜ψ⌝ bp.codeIn bq.codeIn
...
@[simp] lemma formulaSet_codeIn_finset (Γ : Finset (SyntacticFormula L)) : IsFormulaSet L (⌜Γ⌝ : V) := by
  intro x hx
...
lemma quote_image_shift [L.DecidableEq] (Γ : Finset (SyntacticFormula L)) :
    setShift L (⌜Γ⌝ : V) = ⌜Γ.image Rewriting.shift⌝ := by
  induction Γ using Finset.induction
  case empty => simp
  case insert φ Γ _ ih => simp [ih]

@[simp] lemma derivation_quote [L.DecidableEq] {Γ : Finset (SyntacticFormula L)} (d : T ⊢₂ Γ) : T.Derivation (⌜d⌝ : V) := by
  induction d
  case closed φ hp hn =>
    exact Theory.Derivation.axL (by simp)
      (by simp [Sequent.mem_codeIn_iff, hp])
      (by rw [←quote_neg, Sequent.mem_codeIn_iff]; simp [hn])
  case root Δ φ hT hp =>
    apply Theory.Derivation.root (by simp)
      (by simp [Sequent.mem_codeIn_iff, hp])
      (by simp [hT])
  case verum Δ h =>
    exact Theory.Derivation.verumIntro (by simp)
      (by simpa [quote_verum] using (Sequent.mem_codeIn_iff (V := V)).mpr h)
  case and Δ φ ψ hpq dp dq ihp ihq =>
    apply Theory.Derivation.andIntro
  ...
  case shift Δ d ih =>
    ...
    using Theory.Derivation.shiftRule ⟨by simp [fstidx_quote], ih⟩
  case cut Δ φ d dn ih ihn =>
    apply Theory.Derivation.cutRule
      ⟨by simp [fstidx_quote], ih⟩
      ⟨by simp [fstidx_quote], ihn⟩

@[simp] lemma derivatioNOf_quote {Γ : Finset (SyntacticFormula L)} (d : T ⊢₂ Γ) : T.DerivationOf (V := V) ⌜d⌝ ⌜Γ⌝ :=
  ⟨by simp, by simp⟩

lemma derivable_of_quote {Γ : Finset (SyntacticFormula L)} (d : T ⊢₂ Γ) : T.Derivable (⌜Γ⌝ : V) := ⟨⌜d⌝, by simp⟩
...
/-- Hilbert–Bernays provability condition D1 -/
theorem provable_of_provable_arith {φ} : T ⊢! φ → T.Provable (⌜φ⌝ : V) :=
...
theorem internal_provable_of_outer_provable_arith {φ} : T ⊢! φ → T.internalize V ⊢! ⌜φ⌝ :=
  internal_provable_of_outer_provable

theorem internal_sentence_provable_of_outer_sentence_provable_arith {σ} : T ⊢!. σ → T.internalize V ⊢! ⌜σ⌝ := fun h ↦
  internal_provable_of_outer_provable_arith (T := T) (V := V) <| Axiom.provable_iff.mp h
...
lemma _root_.LO.FirstOrder.Theory.Derivation.sound {d : ℕ} (h : T.Derivation d) : ∃ Γ, ⌜Γ⌝ = fstIdx d ∧ T ⊢₂! Γ := by
...
lemma _root_.LO.FirstOrder.Theory.Provable.sound2 {φ : SyntacticFormula L} (h : T.Provable (⌜φ⌝ : ℕ)) : T ⊢₂.! φ := by
...
lemma _root_.LO.FirstOrder.Theory.Provable.sound {φ : SyntacticFormula L} (h : T.Provable (⌜φ⌝ : ℕ)) : T ⊢! φ :=
  provable_iff_derivable2.mpr <| Theory.Provable.sound2 (by simpa using h)

lemma _root_.LO.FirstOrder.Theory.Provable.smallSound {σ : Sentence L} (h : T.Provable (⌜σ⌝ : ℕ)) : T ⊢! ↑σ :=
  provable_iff_derivable2.mpr <| Theory.Provable.sound2 (by simpa using h)
