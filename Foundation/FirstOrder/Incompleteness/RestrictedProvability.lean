...
def RestrictedProvable (𝔢 : ℕ) (T : Theory L) [T.Δ₁] (φ : V) := ∃ d ≤ (ORingStructure.numeral 𝔢), T.Proof d φ

noncomputable def restrictedProvable (𝔢 : ℕ) : 𝚺₁.Semisentence 1 := .mkSigma “φ. ∃ d, d ≤ !𝔢 ∧ !T.proof.sigma d φ”

noncomputable abbrev restrictedProvabilityPred (𝔢 : ℕ) (σ : Sentence L) : ArithmeticSentence := (T.restrictedProvable 𝔢).val/[⌜σ⌝]

instance RestrictedProvable.defined {𝔢} : 𝚺₁-Predicate[V] T.RestrictedProvable 𝔢 via T.restrictedProvable 𝔢 where
...
@[simp]
lemma restrictedGödel_sigma_one {𝔢 : ℕ} : Hierarchy 𝚺 1 (T.restrictedGödel 𝔢) := by
...
example [T.SoundOnHierarchy 𝚺 1] [𝗥₀ ⪯ T] : ℕ ⊧ₘ T.restrictedGödel (10^12) ∧ T ⊢ T.restrictedGödel (10^12) := by
  constructor;
