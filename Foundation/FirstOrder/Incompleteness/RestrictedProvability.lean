...
natural number). The results with `f = Superexp.superexp` recover "provable by proof whose Gödel
number is less than the superexponential of `e`".
-/
...
lemma def_restrictedGödel [𝗜𝚺₁ ⪯ U] :
    U ⊢ T.restrictedGödel fDef e 🡘
      (∼(T.restrictedProvable fDef e).val)/[⌜T.restrictedGödel fDef e⌝] :=
  diagonal _

private lemma def_restrictedGödel' [𝗜𝚺₁ ⪯ U] :
    U ⊢ T.restrictedGödel' fDef e 🡘
      (∼(T.restrictedProvable fDef e).val)/[⌜T.restrictedGödel fDef e⌝] := by simp;

private lemma provable_E_restrictedGödel_restrictedGödel' [𝗜𝚺₁ ⪯ U] :
    U ⊢ T.restrictedGödel fDef e 🡘 T.restrictedGödel' fDef e := by
  apply Entailment.E_trans;
  · exact def_restrictedGödel;
  · exact Entailment.E_symm <| def_restrictedGödel';

private lemma iff_provable_restrictedGödel_provable_restrictedGödel' [𝗜𝚺₁ ⪯ U] :
    U ⊢ (T.restrictedGödel fDef e) ↔ U ⊢ (T.restrictedGödel' fDef e) := by
  apply Entailment.iff_of_E provable_E_restrictedGödel_restrictedGödel';

private lemma iff_true_restrictedGödel_true_restrictedGödel' :
    ℕ↓[ℒₒᵣ] ⊧ (T.restrictedGödel fDef e) ↔ ℕ↓[ℒₒᵣ] ⊧ (T.restrictedGödel' fDef e) := by
...
lemma models_restrictedGödel (f : V → V) [𝚺₁-Function₁[V] f via fDef] :
    V↓[ℒₒᵣ] ⊧ T.restrictedGödel fDef e ↔ ∀ x : V, x < f (ORingStructure.numeral e) →
      ¬Arithmetic.Bootstrapping.Proof T x (⌜T.restrictedGödel fDef e⌝) := by
  apply Iff.trans <| Semantics.models_iff.mp <|
...
theorem two_pow_le_superexp {e : ℕ} (he : 1 ≤ e) : 2 ^ e ≤ Superexp.superexp e := by
  have h1 : iterExp e 1 = 2 ^ e :=
    (iterExp_succ e 0).trans (by rw [iterExp_zero]; exact exp_nat_eq_two_pow e)
...
theorem lower_bound_gödelNumber_proof_restrictedGödel_superexp {e : ℕ} :
  ...
  simpa [numeral_eq_natCast] using
    lower_bound_gödelNumber_proof_restrictedGödel Superexp.superexp (e := e)
...
example :
  letI e : ℕ := 10^9;
   T ⊢ T.restrictedGödel superexpDef e ∧ ∀ b : T ⊢! T.restrictedGödel superexpDef e,
     Superexp.superexp e ≤ ⌜b⌝  := by
  constructor;
