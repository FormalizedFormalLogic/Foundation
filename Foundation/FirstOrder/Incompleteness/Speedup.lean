...
noncomputable def _root_.FFL.FirstOrder.Theory.minProof
    (T : Theory L) [T.Δ₁] (σ : Sentence L) : ℕ :=
  sInf (Set.range fun d : T ⊢₂! (σ : Proposition L) ↦ (⌜d⌝ : ℕ))

@[grind →]
lemma proof_minProof (h : T ⊢ σ) : Proof T (T.minProof σ) ⌜σ⌝ := by
  obtain ⟨d, hd⟩ : T.minProof σ ∈ Set.range (fun d : T ⊢₂! (σ : Proposition L) ↦ (⌜d⌝ : ℕ)) :=
...
lemma minProof_eq_zero_of_unprovable (h : T ⊬ σ) : T.minProof σ = 0 := by
  have : IsEmpty (T ⊢₂! (σ : Proposition L)) :=
    not_nonempty_iff.mp fun hd ↦ h (provable_iff_derivable2.mpr hd)
...
lemma computablePred_proof : ComputablePred fun p : ℕ × ℕ ↦ Proof T p.1 p.2 := by
  apply ComputablePred.computable_of_manyOneReducible (q := fun n : ℕ => Proof T (π₁ n) (π₂ n));
  · use (fun p => Nat.pair p.1 p.2);
    and_intros;
...
lemma computablePred_bddExists_proof [L.Primcodable] (hF : Computable F) {bd : α → ℕ}
    (hbd : Computable bd) :
  ComputablePred fun a ↦ ∃ d ≤ bd a, Proof T d ⌜F a⌝ := by
  apply ComputablePred.computable_of_manyOneReducible
    (q := fun n : ℕ ↦ ∃ d ≤ π₁ n, Proof T d (π₂ n));
  · use fun a => Nat.pair (bd a) (encode (F a));
    and_intros;
...
lemma computablePred_provable_of_minProof_le [L.Primcodable] (hF : Computable F) {bd : α → ℕ}
  ...
  ComputablePred fun a ↦ T ⊢ F a := by
  apply ComputablePred.of_eq (computablePred_bddExists_proof (T := T) hF hbd);
  intro a;
  have hp : ∀ d, Proof T d ⌜F a⌝ → T ⊢ F a := fun d hd ↦ provable_iff_provable.mp ⟨d, hd⟩;
  grind;
...
private lemma computable_quote_speedupProof [L.Primcodable] :
    Computable fun π ↦ (⌜speedupProof T σ π⌝ : ℕ) :=
  have hc : Computable₂ fun s p : ℕ ↦
      orIntro (insert (s ^⋎ p) ∅) s p (axm (insert s (insert p (insert (s ^⋎ p) ∅))) s) :=
    computable₂_iff_sigma1.mpr (by definability);
  (hc.comp (Computable.const ⌜σ⌝) Sentence.primrec_quote_natCast.to_comp).of_eq fun π ↦ by
...
theorem ehrenfeucht_mycielski_speedup [L.Primcodable]
  ...
  contrapose! hU;
  apply ComputablePred.of_eq ?_ (fun π ↦ provable_insert_neg_iff_or.symm);
  exact computablePred_provable_of_minProof_le
    ...
    fun π hπ ↦ (hU (σ ⋎ π) hπ).trans (Nat.le_boundedMax (minProof_or_le_speedupProof π));
...
example (hσ : T ⊬ σ) :
  ...
  obtain ⟨π, hπ, hlt⟩ := ehrenfeucht_mycielski_speedup_arithmetic hσ (fun x ↦ 2 ^ (x + 1)) <|
