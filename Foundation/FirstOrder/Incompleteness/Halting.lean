...
-- `ComputablePred.computable_iff_re_compl_re` needs decidability of an arbitrary predicate.
open scoped Classical in
lemma incomplete_of_REPred_not_ComputablePred_Nat' {P : ℕ → Prop} (hRE : REPred P)
    (hC : ¬ComputablePred P) :
  ...
  have ⟨d, hd⟩ : ∃ d : ℕ, ¬(¬P d ↔ T ⊢ ∼φ/[d]) := by
    ...
    · suffices REPred fun a : ℕ ↦ T ⊬ φ/[a] by simpa [hP] using! this;
      ...
      · rintro hP
        ...
        simpa [Sentence.quote_def, Semiformula.quote_def, Rewriting.emb_subst_eq_subst_coe₁]
          using hP;
      · rintro hφ
...
-- `computablePred_iff_computable_decide` needs decidability of an arbitrary predicate.
open scoped Classical in
...
/-- If an r.e. but not recursive predicate `P` on a `Primcodable` type exists,
then incompleteness follows. -/
...
/-- The halting problem is r.e. but not recursive, so Gödel's first incompleteness theorem
follows. -/
