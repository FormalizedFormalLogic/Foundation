...
lemma incomplete_of_REPred_not_ComputablePred_Nat' {P : ℕ → Prop} (hRE : REPred P) (hC : ¬ComputablePred P) :
  ...
  have ⟨d, hd⟩ : ∃ d : ℕ, ¬(¬P d ↔ T ⊢ ∼φ/[d]) := by
    ...
    constructor;
    . assumption;
    . suffices REPred fun a : ℕ ↦ T ⊬ φ/[a] by rw [hP]; exact this;
      ...
      intro a;
      push Not at h;
      apply Iff.trans ?_ $ show T ⊢ ∼φ/[a] ↔ ¬T ⊢ φ/[a] by have e := h a |>.symm; rw [hP] at e; exact e;
      constructor;
      . rintro hP
  ...
  . exfalso;
    ...
    replace hd₁ : T ⊢ φ/[d] := by rw [hP] at hd₁; exact hd₁;
    cl_prover [hd₁, hd₂];
