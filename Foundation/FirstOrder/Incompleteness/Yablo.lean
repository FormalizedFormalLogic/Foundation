...
def YabloSystem (T : ArithmeticTheory) [T.Δ₁] (φ n : V) : Prop := ∀ m, n < m → ¬Provable T (substNumeral φ m)

noncomputable def yabloSystem (T : ArithmeticTheory) [T.Δ₁] : 𝚷₁.Semisentence 2 := .mkPi
  “φ n. ∀ m, n < m → ∀ nσ, !ssnum nσ φ m → ¬!(provable T) (nσ)”
...
lemma iff_yablo_provable (n : ℕ) : U ⊢ T.yabloPred n ↔ U ⊢ “∀ m, ↑n < m → ∀ nσ, !ssnum nσ ⌜T.yablo⌝ m → ¬!(provable T) (nσ)” := by
  suffices U ⊢ T.yablo/[n] 🡘 “∀ m, ↑n < m → ∀ nσ, !ssnum nσ ⌜T.yablo⌝ m → ¬!(provable T) (nσ)” by
    constructor <;> . intro h; cl_prover [h, this];
  apply complete.{0} U _;
  intro V _ _;
...
lemma provable_greater_yablo {n m : ℕ} (hnm : n < m) : U ⊢ T.yabloPred n 🡒 T.yabloPred m := by
  ...
  intro h
  have hn : V ⊧/![ORingStructure.numeral n] T.yablo := by simpa [models_iff] using h
  have hm : V ⊧/![ORingStructure.numeral m] T.yablo := (yablo_diagonal_modeled (T := T) (n := (ORingStructure.numeral m : V))).mpr <| by
    intro k hmk
    exact (yablo_diagonal_modeled (T := T) (n := (ORingStructure.numeral n : V))).mp hn k
      (Arithmetic.lt_trans _ _ _ (by simpa) hmk)
  simpa [models_iff] using hm
...
theorem yablo_unprovable [Entailment.Consistent T] : T ⊬ (T.yabloPred n) := by
  ...
  have H₂ : T ⊢ ∼provabilityPred T (T.yabloPred (n + 1)) := by
    apply complete.{0} T _;
    intro V _ _;
    haveI : V↓[ℒₒᵣ] ⊧* 𝗜𝚺₁ := ModelsTheory.of_provably_subtheory V 𝗜𝚺₁ T inferInstance;
    suffices ¬Provable T (substNumeral ⌜T.yablo⌝ (n + 1 : V)) by
      simpa [provabilityPred, models_iff, ←substNumeral_app_quote_nat_model];
    have : ∀ (x : V), ORingStructure.numeral n < x → ¬Provable T (substNumeral ⌜T.yablo⌝ x) := by
      have : V↓[ℒₒᵣ] ⊧ “∀ m, ↑n < m → ∀ nσ, !ssnum nσ ⌜T.yablo⌝ m → ¬!(provable T) (nσ)” :=
