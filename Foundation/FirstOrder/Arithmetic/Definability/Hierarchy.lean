...
protected inductive Semiformula : HierarchySymbol → Type _ where
  | mkSigma {m} (φ : ArithmeticSemiformula ξ n) (hφ : Hierarchy 𝚺 m φ := by simp) :
      𝚺-[m].Semiformula
...
@[simp] lemma val_mkSigma (φ : ArithmeticSemiformula ξ n) (hp : Hierarchy 𝚺 m φ) :
    (mkSigma φ hp).val = φ := rfl

@[simp] lemma val_mkPi (φ : ArithmeticSemiformula ξ n) (hp : Hierarchy 𝚷 m φ) :
    (mkPi φ hp).val = φ := rfl

@[simp] lemma val_mkDelta (φ : 𝚺-[m].Semiformula ξ n) (ψ : 𝚷-[m].Semiformula ξ n) :
...
@[simp] lemma sigma_mkDelta (φ : 𝚺-[m].Semiformula ξ n) (ψ : 𝚷-[m].Semiformula ξ n) :
    (mkDelta φ ψ).sigma = φ := rfl

def pi : 𝚫-[m].Semiformula ξ n → 𝚷-[m].Semiformula ξ n
  | mkDelta _ φ => φ

@[simp] lemma pi_mkDelta (φ : 𝚺-[m].Semiformula ξ n) (ψ : 𝚷-[m].Semiformula ξ n) :
...
@[simp] lemma val_mkPolarity (φ : ArithmeticSemiformula ξ n) {Γ} (h : Hierarchy Γ m φ) :
...
@[simp] lemma ProperOn.rew {φ : 𝚫-[m].Semisentence n₁} (h : φ.ProperOn M)
    (ω : Rew ℒₒᵣ Empty n₁ Empty n₂) : (φ.rew ω).ProperOn M := by
...
@[simp] lemma ProperOn.rew' {φ : 𝚫-[m].Semisentence n₁} (h : φ.ProperOn M)
    (ω : Rew ℒₒᵣ Empty n₁ M n₂) : (φ.rew ω).ProperWithParamOn M := by
...
@[simp] lemma ProperWithParamOn.rew {φ : 𝚫-[m].Semiformula M n₁}
    (h : φ.ProperWithParamOn M) (f : Fin n₁ → ArithmeticSemiterm M n₂) :
    (φ.rew (Rew.subst f)).ProperWithParamOn M := by
  rcases φ; intro e;
...
def ofDeltaOne (φ : 𝚫₁.Semiformula ξ k) : (Γ : SigmaPiDelta) → (m : ℕ) → Γ-[m+1].Semiformula ξ k
  ...
  | 𝚫, m => mkDelta (mkSigma φ.sigma.val (φ.sigma.sigma_prop.mono (by simp)))
...
@[simp] lemma ProperWithParamOn.of_zero (φ : Γ'-[0].Semiformula M k) (m) :
    (ofZero φ 𝚫-[m]).ProperWithParamOn M := by
...
def and : {Γ : HierarchySymbol} → Γ.Semiformula ξ n → Γ.Semiformula ξ n → Γ.Semiformula ξ n
  ...
  | 𝚫-[m], φ, ψ => mkDelta (mkSigma (φ.sigma.val ⋏ ψ.sigma.val) (by simp))
    (mkPi (φ.pi.val ⋏ ψ.pi.val) (by simp))
...
def negDelta (φ : 𝚫-[m].Semiformula ξ n) : 𝚫-[m].Semiformula ξ n :=
...
def ball (t : ArithmeticSemiterm ξ n) :
  ...
  | 𝚫-[m], φ =>
    mkDelta (mkSigma (∀¹[“#0 < !!(Rew.bShift t)”] φ.sigma.val) (by simp))
      (mkPi (∀¹[“#0 < !!(Rew.bShift t)”] φ.pi.val) (by simp))
...
def all (φ : 𝚷-[m + 1].Semiformula ξ (n + 1)) : 𝚷-[m + 1].Semiformula ξ n :=
  mkPi (∀¹ φ.val) φ.pi_prop.all

def exs (φ : 𝚺-[m + 1].Semiformula ξ (n + 1)) : 𝚺-[m + 1].Semiformula ξ n :=
  mkSigma (∃¹ φ.val) φ.sigma_prop.exs
...
@[simp] lemma val_negSigma {m} (φ : 𝚺-[m].Semiformula ξ n) : φ.negSigma.val = ∼φ.val := by
  simp [negSigma]

@[simp] lemma val_negPi {m} (φ : 𝚷-[m].Semiformula ξ n) : φ.negPi.val = ∼φ.val := by simp [negPi]

lemma val_negDelta {m} (φ : 𝚫-[m].Semiformula ξ n) : (∼φ).val = ∼φ.pi.val := by
  simp [HTilde.hTilde, Tilde.tilde, negDelta]

@[simp] lemma sigma_negDelta {m} (φ : 𝚫-[m].Semiformula ξ n) : (∼φ).sigma = φ.pi.negPi := by
  simp [HTilde.hTilde, Tilde.tilde, negDelta]

@[simp] lemma sigma_negPi {m} (φ : 𝚫-[m].Semiformula ξ n) : (∼φ).pi = φ.sigma.negSigma := by
  simp [HTilde.hTilde, Tilde.tilde, negDelta]
...
@[simp] lemma val_bexs (t : ArithmeticSemiterm ξ n) (φ : Γ.Semiformula ξ (n + 1)) :
    (bexs t φ).val = ∃¹[“#0 < !!(Rew.bShift t)”] φ.val := by
...
lemma ProperOn.and {φ ψ : 𝚫-[m].Semisentence k} (hp : φ.ProperOn M) (hq : ψ.ProperOn M) :
    (φ ⋏ ψ).ProperOn M := by
  intro e; simp [hp.iff, hq.iff]

lemma ProperOn.or {φ ψ : 𝚫-[m].Semisentence k} (hp : φ.ProperOn M) (hq : ψ.ProperOn M) :
    (φ ⋎ ψ).ProperOn M := by
  intro e; simp [hp.iff, hq.iff]
...
lemma ProperOn.ball {t} {φ : 𝚫-[m + 1].Semisentence (k + 1)} (hp : φ.ProperOn M) :
    (ball t φ).ProperOn M := by
  intro e; simp [Semiformula.ball, hp.iff]

lemma ProperOn.bexs {t} {φ : 𝚫-[m + 1].Semisentence (k + 1)} (hp : φ.ProperOn M) :
    (bexs t φ).ProperOn M := by
  intro e; simp [Semiformula.bexs, hp.iff]

@[simp] lemma ProperWithParamOn.verum : (⊤ : 𝚫-[m].Semiformula M k).ProperWithParamOn M := by
  intro e; simp

@[simp] lemma ProperWithParamOn.falsum : (⊥ : 𝚫-[m].Semiformula M k).ProperWithParamOn M := by
  intro e; simp
...
lemma ProperWithParamOn.neg {φ : 𝚫-[m].Semiformula M k} (hp : φ.ProperWithParamOn M) :
    (∼φ).ProperWithParamOn M := by
  intro e; simp [hp.iff]

lemma ProperWithParamOn.eval_neg {φ : 𝚫-[m].Semiformula M k} (hp : φ.ProperWithParamOn M)
    (e : Fin k → M) :
...
@[simp] lemma graphDelta_val (φ : 𝚺-[m].Semiformula ξ (k + 1)) : φ.graphDelta.val = φ.val := by
