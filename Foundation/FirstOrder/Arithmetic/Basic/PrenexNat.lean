...
lemma coreClosure_one : CoreClosure 1 where
  and := fun Γ {n φ ψ} hφ hψ => by
    ...
    refine ⟨φ' ⋏ ψ', StrictHierarchy.base (Hierarchy.and hφ'.hierarchy hψ'.hierarchy), fun e => ?_⟩
    simp only [LogicalConnective.HomClass.map_and]
    exact and_congr (hiffφ e) (hiffψ e)
  or := fun Γ {n φ ψ} hφ hψ => by
    rcases hφ with ⟨φ', hφ', hiffφ⟩
    rcases hψ with ⟨ψ', hψ', hiffψ⟩
    refine ⟨φ' ⋎ ψ', StrictHierarchy.base (Hierarchy.or hφ'.hierarchy hψ'.hierarchy), fun e => ?_⟩
    simp only [LogicalConnective.HomClass.map_or]
    exact or_congr (hiffφ e) (hiffψ e)
...
lemma bexs_sigma_step (ih : CoreClosure (s + 1)) :
  ...
  simp only [Semiformula.eval_ex, hiffχ, Semiformula.eval_bexsLT, Semiterm.val_bShift, hswap,
    hφ_iff]
  constructor
  · rintro ⟨a, b, hb, hab⟩
    exact ⟨b, hb, a, hab⟩
