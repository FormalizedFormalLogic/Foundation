...
@[grind →]
lemma neg (h : IsStrictHierarchyOnModel ℕ Γ s φ) : IsStrictHierarchyOnModel ℕ Γ.alt s (∼φ) := by
  rcases h with ⟨φ', hφ', hiff'⟩
  refine ⟨∼φ', hφ'.neg, ?_⟩
  intro e
  simp [hiff' e]
...
@[simp]
lemma neg_iff : IsStrictHierarchyOnModel ℕ Γ.alt s (∼φ) ↔ IsStrictHierarchyOnModel ℕ Γ s φ := by
  constructor
  · intro h
    have := h.neg
    simpa using this
...
@[grind →]
lemma alt_up (h : IsStrictHierarchyOnModel ℕ Γ (s + 1) φ) : IsStrictHierarchyOnModel ℕ Γ.alt (s + 2) φ := by
  rcases Γ with _ | _
  · rcases h with ⟨φ', hφ', hiff'⟩
    refine ⟨∀⁰ (Rew.bShift ▹ φ'), (hφ'.rew Rew.bShift).all, ?_⟩
    intro e
    ...
    · intro h _
      exact (hiff' e).mpr h
  · rcases h with ⟨φ', hφ', hiff'⟩
    refine ⟨∃⁰ (Rew.bShift ▹ φ'), (hφ'.rew Rew.bShift).exs, ?_⟩
    intro e
...
@[grind =>]
lemma of_deltaZero (hp : Hierarchy 𝚺 0 φ) (hs : 1 ≤ s) : IsStrictHierarchyOnModel ℕ Γ s φ := by
...
lemma coreClosure_one : CoreClosure 1 where
  ...
  ball := fun Γ {n φ t} ht hφ => by
    ...
    refine ⟨∀⁰[“x. x < !!t”] φ', StrictHierarchy.base (Hierarchy.ball ht hφ'.hierarchy), ?_⟩
    intro e
    simp only [Semiformula.eval_ball]
    exact forall_congr' (fun x => imp_congr Iff.rfl (hiff' (x :> e)))
  bexs := fun Γ {n φ t} ht hφ => by
    rcases hφ with ⟨φ', hφ', hiff'⟩
    refine ⟨∃⁰[“x. x < !!t”] φ', StrictHierarchy.base (Hierarchy.bexs ht hφ'.hierarchy), ?_⟩
    intro e
...
- [HP98, Theorem 0.34] -/
@[grind ←]
lemma and (hs : 1 ≤ s) (hφ : IsStrictHierarchyOnModel ℕ Γ s φ) (hψ : IsStrictHierarchyOnModel ℕ Γ s ψ) : IsStrictHierarchyOnModel ℕ Γ s (φ ⋏ ψ) :=
...
- [HP98, Theorem 0.34] -/
@[grind ←]
lemma or (hs : 1 ≤ s) (hφ : IsStrictHierarchyOnModel ℕ Γ s φ) (hψ : IsStrictHierarchyOnModel ℕ Γ s ψ) : IsStrictHierarchyOnModel ℕ Γ s (φ ⋎ ψ) :=
...
@[grind ←]
lemma exs_of_pi {φ : ArithmeticSemiformula Empty (n + 1)} (h : IsStrictHierarchyOnModel ℕ 𝚷 (s + 1) φ) :
...
@[grind ←]
lemma all_of_sigma {φ : ArithmeticSemiformula Empty (n + 1)} (h : IsStrictHierarchyOnModel ℕ 𝚺 (s + 1) φ) :
...
- [HP98, Theorem 0.34] -/
@[grind ←]
lemma exs {φ : ArithmeticSemiformula Empty (n + 1)} (h : IsStrictHierarchyOnModel ℕ 𝚺 (s + 1) φ) :
  ...
  · -- s + 1 = 1: Σ₁ is already closed under ∃
    ...
    cases hφ' with
    | base hb =>
      refine ⟨∃⁰ φ', StrictHierarchy.base hb.exs, ?_⟩
      intro e
      simp only [Semiformula.eval_ex]
      exact exists_congr (fun x => hiff' (x :> e))
...
- [HP98, Theorem 0.34] -/
@[grind ←]
lemma all {φ : ArithmeticSemiformula Empty (n + 1)} (h : IsStrictHierarchyOnModel ℕ 𝚷 (s + 1) φ) :
...
- [HP98, Theorem 0.34] -/
@[grind →]
lemma ofHierarchy (h : Hierarchy Γ s φ) (hs : 1 ≤ s) : IsStrictHierarchyOnModel ℕ Γ s φ := by
  induction h with
...
lemma mono (h : IsStrictHierarchyOnModel ℕ Γ s φ) (hs : 1 ≤ s) (hle : s ≤ s') : IsStrictHierarchyOnModel ℕ Γ s' φ := by
  rcases h with ⟨φ', hφ', hiff'⟩
...
lemma mono' (h : IsStrictHierarchyOnModel ℕ Γ s φ) (hs : 1 ≤ s) (hlt : s < s') : IsStrictHierarchyOnModel ℕ Γ' s' φ := by
  rcases h with ⟨φ', hφ', hiff'⟩
