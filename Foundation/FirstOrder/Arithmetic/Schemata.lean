...
abbrev ISigmaPlus (s : ℕ) : ArithmeticTheory := 𝗜𝗡𝗗⁺ 𝚺 s

prefix:max "𝗜𝚺⁺" => ISigmaPlus

notation "𝗜𝚺⁺₀" => ISigmaPlus 0

notation "𝗜𝚺⁺₁" => ISigmaPlus 1

abbrev IPiPlus (s : ℕ) : ArithmeticTheory := 𝗜𝗡𝗗⁺ 𝚷 s

prefix:max "𝗜𝚷⁺" => IPiPlus

notation "𝗜𝚷⁺₀" => IPiPlus 0

notation "𝗜𝚷⁺₁" => IPiPlus 1
...
abbrev LSigmaPlus (s : ℕ) : ArithmeticTheory := 𝗟⁺ 𝚺 s

prefix:max "𝗟𝚺⁺" => LSigmaPlus

abbrev LPiPlus (s : ℕ) : ArithmeticTheory := 𝗟⁺ 𝚷 s

prefix:max "𝗟𝚷⁺" => LPiPlus
...
lemma ISigmaPlus_subset_mono {s₁ s₂} (h : s₁ ≤ s₂) : 𝗜𝚺⁺ s₁ ⊆ 𝗜𝚺⁺ s₂ :=
  Set.union_subset_union_right _ (InductionScheme_subset (fun H ↦ H.mono h))

lemma ISigmaPlus_weakerThan_of_le {s₁ s₂} (h : s₁ ≤ s₂) : 𝗜𝚺⁺ s₁ ⪯ 𝗜𝚺⁺ s₂ :=
  Entailment.WeakerThan.ofSubset (ISigmaPlus_subset_mono h)

lemma ISigmaPlus_weakerThan_of_le_trans {T : ArithmeticTheory} {s₁ s₂} (h : s₁ ≤ s₂) (hT : 𝗜𝚺⁺ s₂ ⪯ T) :
    𝗜𝚺⁺ s₁ ⪯ T :=
  Entailment.WeakerThan.trans (ISigmaPlus_weakerThan_of_le h) hT
...
lemma ISigma_zero_eq_ISigmaPlus_zero : 𝗜𝚺₀ = 𝗜𝚺⁺₀ :=
  InductionOnHierarchy_zero_eq_InductionOnBroadHierarchy_zero 𝚺 𝚺

lemma ISigmaZero_subset_ISigmaPlus {s : ℕ} : 𝗜𝚺₀ ⊆ 𝗜𝚺⁺ s :=
  Set.union_subset_union_right _ (InductionScheme_subset fun H ↦ (Arithmetic.StrictHierarchy.zero_iff.mp H).of_zero)

lemma ISigmaPlus_zero_subset_ISigmaZero : 𝗜𝚺⁺₀ ⊆ 𝗜𝚺₀ :=
  le_of_eq ISigma_zero_eq_ISigmaPlus_zero.symm
...
instance : 𝗜𝚺⁺₀ ⪯ 𝗜𝚺⁺₁ := ISigmaPlus_weakerThan_of_le (by decide)

instance : 𝗜𝚺₀ ⪯ 𝗜𝚺⁺ s :=
  Entailment.WeakerThan.ofSubset ISigmaZero_subset_ISigmaPlus

instance (s : ℕ) : 𝗜𝚺⁺₀ ⪯ 𝗜𝚺 s :=
  Entailment.WeakerThan.ofSubset
    (Set.Subset.trans ISigmaPlus_zero_subset_ISigmaZero (ISigma_subset_mono (Nat.zero_le s)))

/-- `𝗜𝚺₀` and `𝗜𝚺⁺₀` are the same theory: at level `0` the two hierarchies both cut out `Δ₀`. -/
instance ISigmaZero_equiv_ISigmaPlusZero : 𝗜𝚺₀ ≊ 𝗜𝚺⁺₀ :=
...
-- This is stated as a `lemma`, not an `instance`, since `s` does not occur in the conclusion
-- `𝗘𝗤 ℒₒᵣ ⪯ T`, so instance search cannot infer it.
lemma eq_weakerThan_of_ISigmaPlus {T : ArithmeticTheory} {s : ℕ} [𝗜𝚺⁺ s ⪯ T] : 𝗘𝗤 ℒₒᵣ ⪯ T :=
  Entailment.WeakerThan.trans (inferInstance : 𝗘𝗤 ℒₒᵣ ⪯ 𝗜𝚺⁺₀) (ISigmaPlus_weakerThan_of_le_trans (by omega) ‹𝗜𝚺⁺ s ⪯ T›)
...
instance [V↓[ℒₒᵣ] ⊧* 𝗜𝚺₀] : V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺₀ :=
  models_of_ss inferInstance ISigmaPlus_zero_subset_ISigmaZero
...
lemma mod_ISigma_of_ISigmaPlus {s} [V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s] : V↓[ℒₒᵣ] ⊧* 𝗜𝚺 s :=
...
lemma mod_ISigmaPlus_of_le {s₁ s₂} (h : s₁ ≤ s₂) [V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s₂] : V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s₁ :=
  models_of_ss inferInstance (ISigmaPlus_subset_mono h)

instance [V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺₁] : V↓[ℒₒᵣ] ⊧* 𝗜𝚺₀ :=
  models_of_ss inferInstance (ISigmaZero_subset_ISigmaPlus (s := 1))

instance [V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺s] : V↓[ℒₒᵣ] ⊧* 𝗜𝚷⁺s := inferInstance

instance [V↓[ℒₒᵣ] ⊧* 𝗜𝚷⁺s] : V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺s := inferInstance

lemma models_ISigmaPlus_iff_models_IPiPlus {s} : V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s ↔ V↓[ℒₒᵣ] ⊧* 𝗜𝚷⁺ s :=
...
abbrev mod_ISigmaPlus_of_le {s₁ s₂} (h : s₁ ≤ s₂) [V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s₂] : V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s₁ :=
  models_of_ss inferInstance (ISigmaPlus_subset_mono h)
...
-- This is stated as a `lemma`, not an `instance`, since `s` does not occur in the conclusion
-- `V↓[ℒₒᵣ] ⊧* 𝗣𝗔⁻`, so instance search cannot infer it.
lemma mod_paMinus_of_ISigmaPlus {s} [V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺ s] : V↓[ℒₒᵣ] ⊧* 𝗣𝗔⁻ :=
  have : V↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺₀ := mod_ISigmaPlus_of_le (Nat.zero_le s)
  have : V↓[ℒₒᵣ] ⊧* 𝗜𝚺₀ := models_of_ss inferInstance (ISigmaZero_subset_ISigmaPlus (s := 0))
  inferInstance
...
instance models_ISigmaPlus (Γ s) : ℕ↓[ℒₒᵣ] ⊧* 𝗜𝗡𝗗⁺ Γ s := by
  have : ∀ φ, ℕ↓[ℒₒᵣ] ⊧ (succInd φ).univCl := models_succInd
  simp only [Semantics.ModelsSet.union_iff, PeanoMinus.instModelsSetStrucORingSentenceStrNat,
    true_and, InductionScheme]
  exact Semantics.ModelsSet.setOf_iff.mpr (fun ψ ⟨φ, _, hψ⟩ => hψ ▸ this φ)
...
instance models_ISigmaPlusOne : ℕ↓[ℒₒᵣ] ⊧* 𝗜𝚺⁺₁ := inferInstance
...
instance sigmaOneSound_ISigmaPlusOne : 𝗜𝚺⁺₁.SoundOnHierarchy 𝚺 1 := inferInstance
