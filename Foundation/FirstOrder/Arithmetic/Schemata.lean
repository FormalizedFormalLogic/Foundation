...
variable {C : Semiformula ℒₒᵣ ℕ 1 → Prop} [V↓[ℒₒᵣ] ⊧* InductionScheme ℒₒᵣ C]

private lemma induction_eval {φ : Semiformula ℒₒᵣ ℕ 1} (hp : C φ) (v : ℕ → V) :
    φ.Eval ![0] v →
...
lemma succ_induction {P : V → Prop}
    (hP : ∃ e : ℕ → V, ∃ φ : Semiformula ℒₒᵣ ℕ 1, C φ ∧ ∀ x, P x ↔ φ.Eval ![x] e) :
...
lemma models_succInd (φ : Semiformula ℒₒᵣ ℕ 1) : ℕ↓[ℒₒᵣ] ⊧ (succInd φ).univCl := by
  suffices
    ∀ f : ℕ → ℕ,
