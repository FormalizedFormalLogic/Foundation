...
@[simp] protected lemma zero_le : ∀ x : M, 0 ≤ x := by
  simpa [models_iff, Structure.le_iff_of_eq_of_lt, le_def] using ModelsTheory.models M PeanoMinus.zeroLe
...
lemma one_le_of_zero_lt : ∀ x : M, 0 < x → 1 ≤ x := by
  simpa [models_iff, Structure.le_iff_of_eq_of_lt, le_def] using ModelsTheory.models M PeanoMinus.oneLeOfZeroLt
...
@[simp] lemma le_mul_self (a : M) : a ≤ a * a := by
  have : 0 ≤ a := by exact Arithmetic.zero_le a
