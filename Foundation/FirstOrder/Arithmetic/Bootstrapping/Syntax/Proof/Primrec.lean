...
lemma primrec₂_nat_insert : Primrec₂ λ x s : ℕ ↦ (insert x s : ℕ) :=
  Primrec₂.of_eq (f := λ x s : ℕ ↦ if s / 2 ^ x % 2 = 1 then s else s + 2 ^ x)
    (by primrec) λ x s ↦ (nat_insert_eq x s).symm
...
lemma primrec_quote_or {h : ∀ x, φ x ⋎ ψ x ∈ Γ x}
    ...
    Primrec λ x ↦ (⌜LK2.Derivation.or (h x) (d x)⌝ : ℕ) :=
...
lemma primrec_quote_axm {hT : ∀ x, ξ x ∈ T} {hm : ∀ x, ((ξ x : Sentence L) : Proposition L) ∈ Γ x}
    ...
    Primrec λ x ↦ (⌜LK2.Derivation.axm (Γ := Γ x) (ξ x) (hT x) (hm x)⌝ : ℕ) :=
