...
lemma primrec₂_nat_insert : Primrec₂ fun x s : ℕ ↦ (insert x s : ℕ) := by
  have hpow : Primrec fun z : ℕ × ℕ ↦ 2 ^ z.1 :=
    (Primrec₂.unpaired'.1 Nat.Primrec.pow).comp (Primrec.const 2) Primrec.fst
  have hc : PrimrecPred fun z : ℕ × ℕ ↦ z.2 / 2 ^ z.1 % 2 = 1 :=
    Primrec.eq.comp
  ...
  exact (Primrec.ite hc Primrec.snd (Primrec.nat_add.comp Primrec.snd hpow)).of_eq
    fun z ↦ (nat_insert_eq z.1 z.2).symm

variable {α : Type*} [Primcodable α] {s p q t d d₁ d₂ : α → ℕ}

lemma primrec_insert (hp : Primrec p) (hq : Primrec q) :
    Primrec fun x ↦ (insert (p x) (q x) : ℕ) := primrec₂_nat_insert.comp hp hq

lemma primrec_axL (hs : Primrec s) (hp : Primrec p) : Primrec fun x ↦ axL (s x) (p x) :=
  (Primrec.succ.comp (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 0) hp))).of_eq
    fun x ↦ by simp [axL, nat_pair_eq]

lemma primrec_verumIntro (hs : Primrec s) : Primrec fun x ↦ verumIntro (s x) :=
  (Primrec.succ.comp
    (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 1) (.const 0)))).of_eq
    fun x ↦ by simp [verumIntro, nat_pair_eq]

lemma primrec_andIntro (hs : Primrec s) (hp : Primrec p) (hq : Primrec q)
    (hd₁ : Primrec d₁) (hd₂ : Primrec d₂) :
    Primrec fun x ↦ andIntro (s x) (p x) (q x) (d₁ x) (d₂ x) :=
...
lemma primrec_exsIntro (hs : Primrec s) (hp : Primrec p) (ht : Primrec t) (hd : Primrec d) :
  ...
  (Primrec.succ.comp (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 5)
    ...
    fun x ↦ by simp [exsIntro, nat_pair_eq]

lemma primrec_wkRule (hs : Primrec s) (hd : Primrec d) : Primrec fun x ↦ wkRule (s x) (d x) :=
  (Primrec.succ.comp (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 6) hd))).of_eq
    fun x ↦ by simp [wkRule, nat_pair_eq]

lemma primrec_shiftRule (hs : Primrec s) (hd : Primrec d) : Primrec fun x ↦ shiftRule (s x) (d x) :=
  (Primrec.succ.comp (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 7) hd))).of_eq
    fun x ↦ by simp [shiftRule, nat_pair_eq]

lemma primrec_cutRule (hs : Primrec s) (hp : Primrec p)
    (hd₁ : Primrec d₁) (hd₂ : Primrec d₂) :
    Primrec fun x ↦ cutRule (s x) (p x) (d₁ x) (d₂ x) :=
  (Primrec.succ.comp (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 8)
    ...
    fun x ↦ by simp [cutRule, nat_pair_eq]

lemma primrec_axm (hs : Primrec s) (hp : Primrec p) : Primrec fun x ↦ axm (s x) (p x) :=
  (Primrec.succ.comp (Primrec₂.natPair.comp hs (Primrec₂.natPair.comp (.const 9) hp))).of_eq
    fun x ↦ by simp [axm, nat_pair_eq]
...
lemma primrec_quote_singleton (hφ : Primrec fun x ↦ (⌜φ x⌝ : ℕ)) :
    Primrec fun x ↦ (⌜({φ x} : Finset (Proposition L))⌝ : ℕ) :=
  (primrec_insert hφ (.const ∅)).of_eq fun x ↦ by simp

lemma primrec_quote_insert (hφ : Primrec fun x ↦ (⌜φ x⌝ : ℕ)) (hΓ : Primrec fun x ↦ (⌜Γ x⌝ : ℕ)) :
    Primrec fun x ↦ (⌜φ x ⫽ Γ x⌝ : ℕ) :=
  (primrec_insert hφ hΓ).of_eq fun x ↦ by simp

lemma primrec_quote_or {h : ∀ x, φ x ⋎ ψ x ∈ Γ x}
    {d : ∀ x, T ⟹₂φ x ⫽ ψ x ⫽ Γ x}
    (hΓ : Primrec fun x ↦ (⌜Γ x⌝ : ℕ)) (hφ : Primrec fun x ↦ (⌜φ x⌝ : ℕ))
    (hψ : Primrec fun x ↦ (⌜ψ x⌝ : ℕ)) (hd : Primrec fun x ↦ (⌜d x⌝ : ℕ)) :
    Primrec fun x ↦ (⌜LK2.Derivation.or (h x) (d x)⌝ : ℕ) :=
  (primrec_orIntro hΓ hφ hψ hd).of_eq fun x ↦ by simp [quote_or]

lemma primrec_quote_axm {hT : ∀ x, ξ x ∈ T} {hm : ∀ x, ((ξ x : Sentence L) : Proposition L) ∈ Γ x}
    (hΓ : Primrec fun x ↦ (⌜Γ x⌝ : ℕ)) (hξ : Primrec fun x ↦ (⌜ξ x⌝ : ℕ)) :
    Primrec fun x ↦ (⌜LK2.Derivation.axm (Γ := Γ x) (ξ x) (hT x) (hm x)⌝ : ℕ) :=
  (primrec_axm hΓ hξ).of_eq fun x ↦ by simp [quote_axm]
