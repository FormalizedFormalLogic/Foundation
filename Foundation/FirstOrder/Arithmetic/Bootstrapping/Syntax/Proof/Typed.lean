...
noncomputable def all' {φ : Semiformula V L 1} (H : ∀¹ φ ∈ Γ) (dp : T ⊢!ᵈᵉʳ insert φ.free Γ.shift) :
    T ⊢!ᵈᵉʳ Γ :=
...
noncomputable def specialize {φ : Semiformula V L 1} (b : T ⊢! ∀¹ φ) (t : Term V L) :
    T ⊢! φ.subst ![t] := TDerivation.specialize b t
...
lemma shift! {φ : Formula V L} (d : T ⊢ φ) : T ⊢ φ.shift :=
  ⟨by simpa using! TDerivation.shift d.get⟩
...
noncomputable def generalize {Γ} {φ : Semiformula V L 1} (d : Γ.map .shift ⊢[T]! φ.free) :
    Γ ⊢[T]! ∀¹ φ := by
  apply Entailment.FiniteContext.ofDef!
  apply generalizeAux
  simpa [conj_shift] using Entailment.FiniteContext.toDef! d

lemma generalize! {Γ} {φ : Semiformula V L 1} (d : Γ.map .shift ⊢[T] φ.free) : Γ ⊢[T] ∀¹ φ :=
  ⟨generalize d.get⟩
...
noncomputable def specializeWithCtx {Γ} {φ : Semiformula V L 1} (d : Γ ⊢[T]! (∀¹ φ)) (t) :
    Γ ⊢[T]! φ.subst ![t] := specializeWithCtxAux d t
...
lemma substItrDisj_right {m : ℕ} {i z : V}
    ...
    A ⊢ φ.subst (𝕹 i :> w) 🡒 φ.substItrDisj w z := Provable.toTProvable <|
...
lemma substItrConj_right_intro {m : ℕ} {ψ} {w : TermVec V ℒₒᵣ m} {φ : Semiformula V ℒₒᵣ (m + 1)}
    {z : V} (h : ∀ i < z, A ⊢ ψ 🡒 φ.subst (𝕹 i :> w)) :
  ...
  · intro i hi
    ...
    have : A ⊢ ∼ψ ⋎ Semiformula.subst (typedNumeral (z - (i + 1)) :> w) φ :=
      h (z - (i + 1)) (by simp [pos_of_gt hi])
    exact TDerivation.orInv (proof_to_tDerivation this.get)

open Classical in
lemma substItrDisj_left_intro {m : ℕ} {ψ} {w : TermVec V ℒₒᵣ m} {φ : Semiformula V ℒₒᵣ (m + 1)}
    {z : V} (h : ∀ i < z, A ⊢ φ.subst (𝕹 i :> w) 🡒 ψ) :
