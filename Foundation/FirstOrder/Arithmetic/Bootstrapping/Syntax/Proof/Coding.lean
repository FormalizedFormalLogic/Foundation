...
omit [L.DecidableEq] in
lemma Sequent.mem_quote [L.DecidableEq] {Γ : Finset (Proposition L)} {x : V} (hx : x ∈ (⌜Γ⌝ : V)) :
    ∃ φ ∈ Γ, x = ⌜φ⌝ := by
...
lemma Sequent.mem_quote_iff' {Γ : Finset (Proposition L)} {x : V} :
    x ∈ (⌜Γ⌝ : V) ↔ (∃ φ ∈ Γ, x = ⌜φ⌝) := by
  constructor
...
lemma quote_def {Γ : Finset (Proposition L)} (d : T ⟹₂Γ) : (⌜d⌝ : V) = (typedQuote V d).val := rfl
...
lemma coe_quote_proof_eq {φ : Sentence L} (d : T ⊢! φ) : (↑(⌜d⌝ : ℕ) : V) = ⌜d⌝ := by
