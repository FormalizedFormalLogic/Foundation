...
lemma ext_eq_zero_of_lt {L S i : V} (h : ‖S‖ ≤ i * ‖L‖) : S{L}[i] = 0 := by
  simp [ext, bexp_eq_zero_of_le h]

@[simp] lemma ext_le_self (L S i : V) : S{L}[i] ≤ S := le_trans (mod_le _ _) (by simp)

lemma ext_graph_aux (z S L i : V) : z = S{L}[i] ↔
    (‖S‖ ≤ i * ‖L‖ → z = 0) ∧
      (i * ‖L‖ < ‖S‖ → ∃ b ≤ S, Exponential (i * ‖L‖) b ∧ z = S / b % (L ⨳ 1)) := by
  rcases show ‖S‖ ≤ i * ‖L‖ ∨ i * ‖L‖ < ‖S‖ from le_or_gt _ _ with (le | lt)
  · simp [ext_eq_zero_of_lt le, le, not_lt.mpr le]
  · suffices z = S / bexp S (i * ‖L‖) % L ⨳ 1 ↔
        ∃ b ≤ S, Exponential (i * ‖L‖) b ∧ z = S / b % L ⨳ 1 by
...
lemma ext_graph (z S L i : V) : z = S{L}[i] ↔
    ∃ lS ≤ S, lS = ‖S‖ ∧ ∃ lL ≤ L, lL = ‖L‖ ∧
      (lS ≤ i * lL → z = 0) ∧
      (i * lL < lS →
        ∃ b ≤ S, Exponential (i * lL) b ∧
          ∃ hL ≤ 2 * L + 1, Exponential lL hL ∧ ∃ divS ≤ S, divS = S / b ∧ z = divS % hL) := by
  rw [ext_graph_aux]
  rcases show ‖S‖ ≤ i * ‖L‖ ∨ i * ‖L‖ < ‖S‖ from le_or_gt _ _ with (le | lt)
...
@[simp] lemma cons_app_seven {α : Type*} {n : ℕ} (a : α)
    (s : Fin n.succ.succ.succ.succ.succ.succ.succ → α) : (a :> s) 7 = s 6 := rfl

@[simp] lemma cons_app_eight {α : Type*} {n : ℕ} (a : α)
...
lemma ext_eq_of_gt {L S S' i : V} (h : i * ‖L‖ < ‖S'‖) :
    S / bexp S' (i * ‖L‖) % (L ⨳ 1) = S{L}[i] := by
  rcases show i * ‖L‖ < ‖S‖ ∨ ‖S‖ ≤ i * ‖L‖ from lt_or_ge (i * ‖L‖) ‖S‖ with (lt | le)
...
lemma ext_eq_smash_of_le {I L S i : V} (h : i ≤ ‖I‖) :
    S / bexp (I ⨳ L) (i * ‖L‖) % (L ⨳ 1) = S{L}[i] :=
...
lemma ext_add₁_pow2 {L i S₁ S₂ p : V} (pp : Pow2 p) (h : (i + 1) * ‖L‖ < ‖p‖) :
  ...
  have : bexp (S₁ + S₂ * p) (i * ‖L‖) * L ⨳ 1 ∣ p :=
    Pow2.dvd_of_le (by simpa using bexp_pow2 lt_len) pp
      (this.monotone_le (exponential_of_pow2 pp) (le_log_of_lt_length h))
  rcases this with ⟨q, hq⟩
  calc
    (S₁ + S₂ * p){L}[i]
      = (S₁ + p * S₂) / bexp (S₁ + S₂ * p) (i * ‖L‖) % L ⨳ 1 := by simp [ext, mul_comm S₂]
    _ = (S₁ + bexp (S₁ + S₂ * p) (i * ‖L‖) * (L ⨳ 1 * q * S₂)) /
          bexp (S₁ + S₂ * p) (i * ‖L‖) % L ⨳ 1 := by simp [←mul_assoc, ←hq]
    _ = (S₁ / bexp (S₁ + S₂ * p) (i * ‖L‖) + L ⨳ 1 * q * S₂) % L ⨳ 1 := by
          rw [div_add_mul_self' _ _ (bexp_pos lt_len)]
...
lemma ext_add₁_bexp {I L i j S₁ S₂ : V} (hi : i ≤ ‖I‖) (hij : j < i) :
  ...
  ext_add₁_pow2 (bexp_pow2 <| mul_len_lt_len_smash hi)
    (by rw [len_bexp (mul_len_lt_len_smash hi), lt_succ_iff_le]
        exact mul_le_mul_right (succ_le_iff_lt.mpr hij))
...
lemma ext_add₂_bexp {I L i j S₁ S₂ : V} (hij : i + j ≤ ‖I‖) (hS₁ : ‖S₁‖ ≤ i * ‖L‖) :
  ...
  calc
    ...
    _ = (S₁ + S₂ * bexp (I ⨳ L) (i * ‖L‖)) / bexp (I ⨳ L) (i * ‖L‖) /
      ...
      congr 2; exact bexp_add (by simp [←add_mul, mul_len_lt_len_smash hij])
    _ = S₂ / bexp (I ⨳ L) (j * ‖L‖) % (L ⨳ 1) := by
      congr 2
      rw [div_add_mul_self, div_eq_zero_of_lt] <;> simp [hie.lt_iff_len_le.mpr hS₁, hie.range_pos]
    _ = S₂{L}[j] := ext_eq_smash_of_le (le_trans le_add_self hij)

noncomputable def append (I L S i X : V) : V :=
  S % bexp (I ⨳ L) (i * ‖L‖) + X * bexp (I ⨳ L) (i * ‖L‖)

lemma append_nil (I L S i : V) : append I L S i 0 = S % bexp (I ⨳ L) (i * ‖L‖) := by simp [append]

lemma len_append (I L S : V) {i X} (hi : i ≤ ‖I‖) (hX : 0 < X) :
    ‖append I L S i X‖ = ‖X‖ + i * ‖L‖ := calc
  ‖append I L S i X‖ = ‖X * bexp (I ⨳ L) (i * ‖L‖) + S % bexp (I ⨳ L) (i * ‖L‖)‖ := by
    simp [append, add_comm]
  _                  = ‖X‖ + log (bexp (I ⨳ L) (i * ‖L‖))                        :=
    length_mul_pow2_add_of_lt hX
      (bexp_pow2 <| mul_len_lt_len_smash hi)
      (mod_lt _ <| bexp_pos <| mul_len_lt_len_smash hi)
  _                  = ‖X‖ + i * ‖L‖                                             := by
    simp [log_bexp (mul_len_lt_len_smash hi)]

lemma append_lt_smash (I L S : V) {i X} (hi : i < ‖I‖) (hX : ‖X‖ ≤ ‖L‖) :
    append I L S i X < I ⨳ L := by
  rcases Arithmetic.zero_le X with (rfl | pos)
  · simpa [append_nil]
      using lt_of_lt_of_le (mod_lt _ (bexp_pos <| mul_len_lt_len_smash <| le_of_lt hi)) (by simp)
  · suffices ‖X‖ + i * ‖L‖ ≤ ‖I‖ * ‖L‖ by
      simpa [lt_smash_iff, len_append I L S (le_of_lt hi) pos]
    calc
      ‖X‖ + i * ‖L‖ ≤ (i + 1) * ‖L‖ := by simp [add_mul, add_comm (i * ‖L‖), hX]
      _             ≤ ‖I‖ * ‖L‖     := mul_le_mul_right (succ_le_iff_lt.mpr hi)

lemma append_lt_sq_smash (I L S : V) {i X} (hi : i ≤ ‖I‖) (hX : ‖X‖ ≤ ‖L‖) (Ipos : 0 < I) :
    append I L S i X < (I ⨳ L) ^ 2 := by
  rcases hi with (rfl | hi)
  · calc
      append I L S ‖I‖ X = S % I ⨳ L + X * I ⨳ L := by simp [append, bexp_eq_smash]
      _                  < (X + 1) * I ⨳ L       := by simp [add_mul, add_comm]
      _                  ≤ L ⨳ 1 * I ⨳ L         :=
        mul_le_mul_right (succ_le_iff_lt.mpr <| lt_smash_one_iff.mpr hX)
      _                  ≤ (I ⨳ L) ^ 2           := by
        simpa [sq, smash_comm L 1] using smash_monotone (pos_iff_one_le.mp Ipos) (by rfl)
  · exact lt_of_lt_of_le (append_lt_smash I L S hi hX) (by simp)

lemma ext_append_last (I L S : V) {i X} (hi : i ≤ ‖I‖) (hX : ‖X‖ ≤ ‖L‖) :
    (append I L S i X){L}[i] = X := calc
  (append I L S i X){L}[i]
    = (S % bexp (I ⨳ L) (i * ‖L‖) + X * bexp (I ⨳ L) (i * ‖L‖)){L}[i + 0] := by simp [append]
...
lemma ext_append_lt (I L S : V) {i j X} (hi : i ≤ ‖I‖) (hij : j < i) :
  ...
  _ = (S % bexp (I ⨳ L) (i * ‖L‖) + (S / bexp (I ⨳ L) (i * ‖L‖)) * bexp (I ⨳ L) (i * ‖L‖)){L}[j] :=
      Eq.symm <| ext_add₁_bexp hi hij
...
def IsSegment (L A start intv S : V) : Prop :=
  ∀ i < intv, S{L}[i + 1] = S{L}[i] + fbit A (start + i)

def Segment (U L A start intv nₛ nₑ : V) : Prop :=
  ∃ S < U, IsSegment L A start intv S ∧ S{L}[0] = nₛ ∧ S{L}[intv] = nₑ

def IsSeries (U I L A iter T : V) : Prop :=
  ∀ l < iter, Segment U L A (‖I‖ * l) ‖I‖ (T{L}[l]) (T{L}[l + 1])

def Series (U I L A iter n : V) : Prop :=
  ∃ T < U, IsSeries U I L A iter T ∧ T{L}[0] = 0 ∧ T{L}[iter] = n

def SeriesSegment (U I L A k n : V) : Prop :=
  ∃ nₖ ≤ n, Series U I L A (k / ‖I‖) nₖ ∧ Segment U L A (‖I‖ * (k / ‖I‖)) (k % ‖I‖) nₖ n

lemma SeriesSegment.series {U I L A k n : V} (H : SeriesSegment U I L A k n) :
    ∃ T S, IsSeries U I L A (k / ‖I‖) T ∧ IsSegment L A (‖I‖ * (k / ‖I‖)) (k % ‖I‖) S ∧
      T{L}[0] = 0 ∧ T{L}[k / ‖I‖] = S{L}[0] ∧ S{L}[k % ‖I‖] = n := by
  rcases H with ⟨_, _, ⟨T, _, hT, hTₛ, hTₑ⟩, ⟨S, _, hS, rfl, rfl⟩⟩
  exact ⟨T, S, hT, hS, hTₛ, hTₑ, rfl⟩

lemma IsSegment.le_add {L A start intv S : V} (H : IsSegment L A start intv S) :
    ∀ i ≤ intv, S{L}[i] ≤ S{L}[0] + i := by
  intro i
...
-- lemma Segment.refl (U L A start n : V) (hU : n < U) (hn : ‖n‖ ≤ ‖L‖) :
--     Segment U L A start 0 n n :=
--   ⟨n, hU, by intro; simp, ext_zero_eq_self_of_le hn, ext_zero_eq_self_of_le hn⟩

lemma Segment.refl (U L A start n : V) (hL : L ⨳ 1 ≤ U) (hn : ‖n‖ ≤ ‖L‖) :
    Segment U L A start 0 n n :=
  ⟨n, lt_of_lt_of_le (lt_smash_one_iff.mpr hn) hL, by intro; simp,
    ext_zero_eq_self_of_le hn, ext_zero_eq_self_of_le hn⟩

lemma Segment.le_add {U L A start intv nₛ nₑ : V} (H : Segment U L A start intv nₛ nₑ) :
    nₑ ≤ nₛ + intv := by
...
lemma IsSeries.le_add {U I L A iter T : V} (H : IsSeries U I L A iter T) :
    ∀ l ≤ iter, T{L}[l] ≤ T{L}[0] + ‖I‖ * l := by
  intro l
...
lemma Series.uniq {U I L A iter n₁ n₂ : V}
    (H₁ : Series U I L A iter n₁) (H₂ : Series U I L A iter n₂) : n₁ = n₂ := by
  ...
  case succ i IH =>
    ...
    have h₂ : Segment U L A (‖I‖ * i) ‖I‖ (T₁{L}[i]) (T₂{L}[i + 1]) := by
      simpa [IH] using HT₂ i (lt_iff_succ_le.mpr hi)
    exact h₁.uniq h₂
...
lemma SeriesSegment.zero {U I L k : V} (Upos : 0 < U) : SeriesSegment U I L 0 k 0 :=
  ⟨0, by rfl, ⟨0, Upos, fun _ _ ↦ ⟨0, Upos, fun _ _ ↦ by simp, by simp⟩, by simp⟩,
    ⟨0, Upos, fun _ _ ↦ by simp, by simp⟩⟩

lemma SeriesSegment.uniq {U I L A k n₁ n₂ : V}
    (H₁ : SeriesSegment U I L A k n₁) (H₂ : SeriesSegment U I L A k n₂) : n₁ = n₂ := by
...
lemma Segment.succ (hU : (I ⨳ L) ^ 2 ≤ U) {start intv nₛ nₑ : V}
    (H : Segment U L A start intv nₛ nₑ) (hintv : intv < ‖I‖) (hnₛ : ‖nₛ + ‖I‖‖ ≤ ‖L‖) :
  ...
  have le_len_L : ‖S{L}[intv] + fbit A (start + intv)‖ ≤ ‖L‖ := calc
    ...
    _                                    ≤ ‖S{L}[0] + intv + 1‖ :=
      length_monotone <| by simpa using H.le_add intv (by rfl)
    _                                    ≤ ‖S{L}[0] + ‖I‖‖      :=
      length_monotone <| by simp [add_assoc, succ_le_iff_lt, hintv]
    _                                    ≤ ‖L‖                  := hnₛ
  have H' : IsSegment L A start (intv + 1) S' := by
    intro i hi
    rcases show i ≤ intv from lt_succ_iff_le.mp hi with (rfl | hi)
    · calc
        S'{L}[i + 1] = S{L}[i] + fbit A (start + i)  :=
          ext_append_last I L S (succ_le_iff_lt.mpr hintv) le_len_L
        _            = S'{L}[i] + fbit A (start + i) := by
          congr 1; symm; exact ext_append_lt I L S (succ_le_iff_lt.mpr hintv) (by simp)
    · calc
        S'{L}[i + 1] = S{L}[i + 1]                   :=
          ext_append_lt I L S (succ_le_iff_lt.mpr hintv) (by simpa using hi)
        _            = S{L}[i] + fbit A (start + i)  := H i hi
        _            = S'{L}[i] + fbit A (start + i) := by
          congr 1; symm; apply ext_append_lt I L S (succ_le_iff_lt.mpr hintv) (by assumption)
  exact
    ⟨ S',
      lt_of_lt_of_le
        (append_lt_sq_smash I L S (succ_le_iff_lt.mpr hintv) le_len_L
          (by simpa using pos_of_gt hintv))
        hU,
      H',
...
lemma Series.succ (hU : (I ⨳ L) ^ 2 ≤ U) (hIL : ‖‖I‖ ^ 2‖ ≤ ‖L‖)
    {iter n n' : V} (HT : Series U I L A iter n) (HS : Segment U L A (‖I‖ * iter) ‖I‖ n n')
    (hiter : iter < ‖I‖) : Series U I L A (iter + 1) n' := by
  ...
  have hn'L : ‖n'‖ ≤ ‖L‖ := calc
    ...
    _    ≤ ‖‖I‖ ^ 2‖            :=
      length_monotone <| by simpa [sq] using mul_le_mul_right (succ_le_iff_lt.mpr hiter)
    _    ≤ ‖L‖                := hIL
  have hTlast : T'{L}[iter + 1] = n' := ext_append_last I L T (succ_le_iff_lt.mpr hiter) hn'L
  have hTofLt : ∀ l ≤ iter, T'{L}[l] = T{L}[l] :=
    fun l hl ↦ ext_append_lt I L T (succ_le_iff_lt.mpr hiter) (by simp [lt_succ_iff_le, hl])
  ...
  ⟨ T',
    lt_of_lt_of_le
      (append_lt_sq_smash I L T (succ_le_iff_lt.mpr hiter) hn'L (by simpa using pos_of_gt hiter))
      hU,
    HT',
    Eq.trans (ext_append_lt I L T (succ_le_iff_lt.mpr hiter) (by simp)) Tₛ,
    hTlast ⟩

lemma div_mod_succ (a b : V) :
    ((a + 1) / b = a / b + 1 ∧ (a + 1) % b = 0 ∧ a % b + 1 = b) ∨
...
lemma SeriesSegment.succ (hU : (I ⨳ L) ^ 2 ≤ U) (hIL : ‖‖I‖ ^ 2‖ ≤ ‖L‖)
  ...
  have hnₘL : ‖nₘ + ‖I‖‖ ≤ ‖L‖ := by
    ...
    calc
      ...
      _        ≤ ‖‖I‖ ^ 2‖                 :=
        length_monotone <| by simpa [sq] using mul_le_mul_left (lt_iff_succ_le.mp this)
      _        ≤ ‖L‖                     := hIL
  ...
  · have : Segment U L A (‖I‖ * (k / ‖I‖)) ‖I‖ nₘ (n + fbit A k) := by
    ...
    exact ⟨n + fbit A k, by rfl, this, by
      ...
      · calc
          L ⨳ 1 ≤ I ⨳ L     := by
            rw [smash_comm L 1]; apply smash_monotone (pos_iff_one_le.mp Ipos) (by rfl)
          _     ≤ (I ⨳ L) ^ 2 := by simp
          _     ≤ U         := hU
      · calc
...
lemma two_add_two_eq_four : 2 + 2 = (4 : V) := by
  simp [←three_add_one_eq_four, ←two_add_one_eq_three, ←one_add_one_eq_two, add_assoc]

lemma four_mul_eq_two_mul_two_mul (a : V) : 4 * a = 2 * (2 * a) := by
  simp [←two_add_two_eq_four, add_mul, two_mul]

@[simp] lemma two_mul_sqrt_le_self (a : V) : 2 * √a ≤ a + 1 :=
  le_trans (two_mul_le_sq_add_one (√a)) (by simp)

lemma four_mul_smash_self (a : V) : (4 * a) ⨳ (4 * a) ≤ (a ⨳ a) ^ 16 := calc
  (4 * a) ⨳ (4 * a) ≤ ((4 * a) ⨳ (2 * a)) ^ 2 := by
    simp [four_mul_eq_two_mul_two_mul, smash_two_mul_le_sq_smash]
  _                 ≤ ((4 * a) ⨳ a) ^ 4       := by
    simp [pow_four_eq_sq_sq, smash_two_mul_le_sq_smash]
  _                 ≤ ((a ⨳ (2 * a)) ^ 2) ^ 4 := by
    rw [smash_comm (4 * a) a]
...
lemma polyI_smash_self_polybounded {A : V} (pos : 0 < A) :
    (polyI A) ⨳ (polyI A) ≤ (2 * A + 1) ^ 4 := calc
  (polyI A) ⨳ (polyI A) = bexp ((polyI A) ⨳ (polyI A)) ((√‖A‖ + 1) ^ 2) :=
    Eq.symm <| by simpa [sq, len_polyI pos] using bexp_eq_smash (polyI A) (polyI A)
  ...
  _                     ≤ bexp ((2 * A) ⨳ (2 * A)) (4 * (√‖A‖) ^ 2)     := by
    simp [mul_pow, two_pow_two_eq_four]
  _                     = (bexp (A ⨳ 1) ((√‖A‖) ^ 2)) ^ 4               :=
    bexp_four_mul
      (by simp [length_smash, lt_succ_iff_le, ←sq, length_two_mul_of_pos pos, ←two_pow_two_eq_four,
        ←mul_pow])
      (by simp [length_smash, lt_succ_iff_le])
  _                     ≤ (bexp (A ⨳ 1) ‖A‖) ^ 4                        := by
    simpa using
      (bexp_monotone_le (by simp [length_smash, lt_succ_iff_le]) (by simp [length_smash])).mpr
        (by simp)
  _                     = (A ⨳ 1) ^ 4                                   := by
    congr 1; simpa using bexp_eq_smash A 1
  _                     ≤ (2 * A + 1) ^ 4                               := by simp

lemma polyI_smash_polyL_polybounded {A : V} (pos : 0 < A) :
    (polyI A) ⨳ (polyL A) ≤ (2 * A + 1) ^ 64 := calc
  (polyI A) ⨳ (polyL A) ≤ (polyI A) ⨳ (3 * polyI A)         :=
    smash_monotone (by rfl) (by simp [polyL, sq_len_le_three_mul])
  _                     ≤ (4 * polyI A) ⨳ (4 * polyI A)     :=
    smash_monotone (le_mul_of_pos_left <| by simp)
      (mul_le_mul_right <| by simp [←three_add_one_eq_four])
  _                     ≤ ((polyI A) ⨳ (polyI A)) ^ (4 * 4) := by simpa using four_mul_smash_self _
  _                     ≤ ((2 * A + 1) ^ 4) ^ (4 * 4)       := by
    simp only [pow_mul, pow_four_le_pow_four, polyI_smash_self_polybounded pos]
  _                     = (2 * A + 1) ^ 64         := by simp [←pow_mul]

lemma sq_polyI_smash_polyL_polybounded {A : V} (pos : 0 < A) :
    ((polyI A) ⨳ (polyL A)) ^ 2 ≤ polyU A := calc
  ((polyI A) ⨳ (polyL A)) ^ 2 ≤ ((2 * A + 1) ^ 64) ^ 2 := by
    simp [polyI_smash_polyL_polybounded pos]
...
omit [V↓[ℒₒᵣ] ⊧* 𝗜𝚺₀ ∪ 𝝮₁] in
lemma bexs_eq_le_iff {p : V → Prop} {z b : V} :
    (∃ a ≤ z, a = b ∧ p a) ↔ (b ≤ z ∧ p b) :=
  ⟨by rintro ⟨a, hp, rfl, hr⟩; exact ⟨hp, hr⟩, by rintro ⟨hp, hr⟩; exact ⟨b, hp, rfl, hr⟩⟩

omit [V↓[ℒₒᵣ] ⊧* 𝗜𝚺₀ ∪ 𝝮₁] in
lemma bexs_eq_lt_iff {p : V → Prop} {z b : V} :
    (∃ a < z, a = b ∧ p a) ↔ (b < z ∧ p b) :=
  ⟨by rintro ⟨a, hp, rfl, hr⟩; exact ⟨hp, hr⟩, by rintro ⟨hp, hr⟩; exact ⟨b, hp, rfl, hr⟩⟩
...
@[simp] lemma NuonAux.initial_iff (A n : V) : NuonAux A 0 n ↔ n = 0 :=
  ⟨fun h ↦ h.uniq (NuonAux.initial A), by rintro rfl; simp⟩
...
lemma NuonAux.uniq {A k n₁ n₂ : V} (H₁ : NuonAux A k n₁) (H₂ : NuonAux A k n₂) : n₁ = n₂ :=
  SeriesSegment.uniq H₁ H₂

lemma NuonAux.succ {A k n : V} (H : NuonAux A k n) (hk : k ≤ ‖A‖) :
    NuonAux A (k + 1) (n + fbit A k) := by
  rcases Arithmetic.zero_le A with (rfl | pos)
  · rcases show n = 0 from H.uniq (NuonAux.zero k); simp
  exact SeriesSegment.succ (sq_polyI_smash_polyL_polybounded pos) (by simp [polyL])
    (lt_of_le_of_lt hk <| polyI_le pos) H
...
lemma NuonAux.succ_elim {A k n : V} (hk : k ≤ ‖A‖) (H : NuonAux A (k + 1) n) :
    ∃ n', n = n' + fbit A k ∧ NuonAux A k n' := by
  rcases NuonAux.exists hk with ⟨n', H'⟩
  rcases H.uniq (H'.succ hk)
  exact ⟨n', rfl, H'⟩

lemma NuonAux.succ_iff {A k n : V} (hk : k ≤ ‖A‖) :
    NuonAux A (k + 1) (n + fbit A k) ↔ NuonAux A k n := by
  constructor
  · intro H
    rcases NuonAux.exists hk with ⟨n', H'⟩
...
lemma NuonAux.two_mul_add_one {A k n : V} (hk : k ≤ ‖A‖) :
    NuonAux A k n → NuonAux (2 * A + 1) (k + 1) (n + 1) := by
  revert n hk
...
lemma Nuon.nuon_eq_iff {a b : V} : b = nuon a ↔ Nuon a b :=
  Classical.choose!_eq_iff_right (Nuon.exists_unique a)

lemma nuon_bit0 (a : V) : nuon (2 * a) = nuon a := by
  rcases Arithmetic.zero_le a with (rfl | pos)
  · simp
  · have : Nuon (2 * a) (nuon a) := by
      simpa [Nuon, length_two_mul_of_pos pos] using (nuon_nuon a).two_mul (by simp)
    exact this.nuon_eq

lemma nuon_bit1 (a : V) : nuon (2 * a + 1) = nuon a + 1 := by
  have : Nuon (2 * a + 1) (nuon a + 1) := by
    simpa [Nuon, length_two_mul_add_one] using (nuon_nuon a).two_mul_add_one (by simp)
  exact this.nuon_eq
