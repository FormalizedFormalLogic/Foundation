...
def Exponential.Seqₛ.Even (X Y u : V) : Prop :=
  ext (u ^ 2) X = 2 * ext u X ∧ ext (u ^ 2) Y = (ext u Y) ^ 2

def Exponential.Seqₛ.Odd (X Y u : V) : Prop :=
  ext (u ^ 2) X = 2 * ext u X + 1 ∧ ext (u ^ 2) Y = 2 * (ext u Y) ^ 2

def Exponential.Seqₛ (y X Y : V) : Prop :=
  ∀ u ≤ y, u ≠ 2 → PPow2 u → Seqₛ.Even X Y u ∨ Seqₛ.Odd X Y u
...
lemma Exponential.Seqₛ.iff (y X Y : V) :
  ...
  ∀ u ≤ y, u ≠ 2 → PPow2 u →
    ((∃ ext_u_X ≤ X, ext_u_X = ext u X ∧ 2 * ext_u_X = ext (u ^ 2) X) ∧
      (∃ ext_u_Y ≤ Y, ext_u_Y = ext u Y ∧ ext_u_Y ^ 2 = ext (u ^ 2) Y)) ∨
    ((∃ ext_u_X ≤ X, ext_u_X = ext u X ∧ 2 * ext_u_X + 1 = ext (u ^ 2) X) ∧
      (∃ ext_u_Y ≤ Y, ext_u_Y = ext u Y ∧ 2 * ext_u_Y ^ 2 = ext (u ^ 2) Y)) :=
  ⟨by intro H u hu ne2 ppu
   ...
   by intro H u hu ne2 ppu
      rcases H u hu ne2 ppu with
        (⟨⟨_, _, rfl, hx⟩, ⟨_, _, rfl, hy⟩⟩ | ⟨⟨_, _, rfl, hx⟩, ⟨_, _, rfl, hy⟩⟩)
...
lemma append_lt (i X : V) {z} (hz : z < i) : append i X z < i ^ 2 := calc
  ...
  _            ≤ i ^ 2               := by
    simpa [sq, add_comm] using mul_le_mul_of_nonneg_right (lt_iff_succ_le.mp hz) (by simp)
...
lemma Seqₛ.append {z x y X Y i : V} (h : Seqₛ z X Y) (ppi : PPow2 i) (hz : z < i) :
  ...
  · simp only [Even, Odd]
    left; rw [ext_append_of_lt, ext_append_of_lt, ext_append_of_lt, ext_append_of_lt] <;>
      try simp [ppi.sq, ppj.sq, *]
    exact H
  · simp only [Even, Odd]
    right; rw [ext_append_of_lt, ext_append_of_lt, ext_append_of_lt, ext_append_of_lt] <;>
      try simp [ppi.sq, ppj.sq, *]
    exact H
...
lemma pow2_ext_of_seq₀_of_seqₛ {y X Y : V} (h₀ : Exponential.Seq₀ X Y) (hₛ : Exponential.Seqₛ y X Y)
  ...
  case ind i IH =>
    ...
    · have ppsq : Pow2 (ext (√i) Y) :=
        IH (√i) (sqrt_lt_self_of_one_lt ppi.one_lt) (ppi.sqrt_ne_two ne2 ei)
...
lemma le_sq_ext_of_seq₀_of_seqₛ {y X Y : V}
    (h₀ : Exponential.Seq₀ X Y) (hₛ : Exponential.Seqₛ y X Y)
...
lemma two_mul_ext_le_of_seq₀_of_seqₛ {y X Y : V}
    (h₀ : Exponential.Seq₀ X Y) (hₛ : Exponential.Seqₛ y X Y)
...
lemma exponential_exists_sq_of_exponential_even {x y : V} :
  ...
  · have hXx : x = ext (√i) X := by simpa [ppi.sq_sqrt_eq ne2, hXx] using hXi
    ...
    have bX' : X' ≤ (ext (√i) Y) ^ 4 := by
      simpa [pow_four_eq_sq_sq, ←hYy] using le_trans (le_of_lt <| by simp [X', ppi.pos]) hi
    have bY' : Y' ≤ (ext (√i) Y) ^ 4 := by
      simpa [pow_four_eq_sq_sq, ←hYy] using le_trans (le_of_lt <| by simp [Y', ppi.pos]) hi
...
lemma bit_one {x y : V} : Exponential x y → Exponential (2 * x + 1) (2 * y ^ 2) := by
  ...
  have hseqₛ' : Seqₛ (2 * y ^ 2) X' Y' := by
    ...
    · have : i = j := by
      ...
      rcases this with rfl
      right
      suffices ext (i ^ 2) X' = 2 * ext i X' + 1 ∧ ext (i ^ 2) Y' = 2 * ext i Y' ^ 2 by
        simpa [Seqₛ.Odd]
      constructor
      · calc
...
lemma two_le_ext_of_seq₀_of_seqₛ {y X Y : V}
    (h₀ : Exponential.Seq₀ X Y) (hₛ : Exponential.Seqₛ y X Y)
...
lemma ext_le_ext_of_seq₀_of_seqₛ {y X Y : V}
    (h₀ : Exponential.Seq₀ X Y) (hₛ : Exponential.Seqₛ y X Y)
  ...
  case ind i IH =>
    ...
    · have IH : ext (√i) X < ext (√i) Y :=
        IH (√i) (sqrt_lt_self_of_one_lt ppi.one_lt) (ppi.sqrt_ne_two ne2 ne4)
          (le_trans (by simp) hi) (ppi.sqrt ne2)
      have twole : 2 ≤ ext (√i) Y :=
        two_le_ext_of_seq₀_of_seqₛ h₀ hₛ (ppi.sqrt_ne_two ne2 ne4) (le_trans (by simp) hi)
          (ppi.sqrt ne2)
      rcases show Seqₛ.Even X Y (√i) ∨ Seqₛ.Odd X Y (√i) from
        hₛ (√i) (sqrt_le_of_le_sq <| hi) (ppi.sqrt_ne_two ne2 ne4) (ppi.sqrt ne2)
          with (heven | hodd)
      · calc
