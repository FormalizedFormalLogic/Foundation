...
def Entailment.paddingIff [Entailment (Formula L ξ) S] {𝓢 : S} [Entailment.Minimal 𝓢] (φ k) :
    𝓢 ⊢ φ.padding k ⭤ φ := by
  apply iffIntro
  · apply and₁
  · apply implyAnd
...
@[simp] def Entailment.padding_iff [Entailment (Formula L ξ) S] {𝓢 : S} [Entailment.Minimal 𝓢] (φ k) :
