...
@[simp] lemma fal {p : ℙ} {φ : Semipropositionᵢ L 1} : p ⊩ ∀¹ φ ↔ ∀ t : SyntacticTerm L, p ⊩ φ/[t] := by
  constructor
  · rintro ⟨b⟩ t
    exact ⟨b.allEquiv t⟩
  · rintro h
...
@[simp] lemma exs {p : ℙ} {φ : Semipropositionᵢ L 1} : p ⊩ ∃¹ φ ↔ ∃ t : SyntacticTerm L, p ⊩ φ/[t] := by
  constructor
  · rintro ⟨b⟩
    have ⟨t, f⟩ := b.exsEquiv
    exact ⟨t, ⟨f⟩⟩
...
@[simp] lemma all {φ : Semiproposition L 1} {p : ℙ} : p ⊩ᶜ ∀¹ φ ↔ ∀ t : SyntacticTerm L, p ⊩ᶜ φ/[t] := by
  simp [iff_isForced, Semiformula.subst_doubleNegation]

@[simp] lemma exs {φ : Semiproposition L 1} {p : ℙ} : p ⊩ᶜ ∃¹ φ ↔ ∀ q ≤ p, ∃ r ≤ q, ∃ t : SyntacticTerm L, r ⊩ᶜ φ/[t] := by
