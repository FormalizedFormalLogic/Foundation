...
lemma consistent_of_model [_hM : M↓[L] ⊧* T] :
    Entailment.Consistent T := Theory.consistent_of_satisfiable ⟨M↓[L], _hM⟩

variable {M}

lemma unprovable_of_countermodel [_hM : M↓[L] ⊧* T] {φ} : M↓[L] ⊭ φ → T ⊬ φ := by
  contrapose!; intro h
  exact Theory.Proof.sound h _hM
