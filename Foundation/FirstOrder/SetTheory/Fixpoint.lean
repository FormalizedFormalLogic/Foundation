...
def limSeqDef : SetTheorySemisentence (k + 2) := (φ.prBlueprint).resultDef

def fixpointDef : SetTheorySemisentence (k + 1) :=
  “x. ∃ s L, !φ.limSeqDef L s ⋯  ∧ x ∈ L”

/- TODO: Once the Lèvy hierarchy has been added, add a `Δ₁` version. -/
-- def fixpointDefΔ₁ : SetTheorySemisentence (k + 1) :=
--   “x. ∃ L, !φ.limSeqDef L (x + 1) ⋯  ∧ x ∈ L”
...
lemma succ_existsUnique (s ih : V) :
  ...
  have : ℒₛₑₜ-predicate fun x ↦ x ≤ s ∧ c.Φ v {z | z ∈ ih} x := by
