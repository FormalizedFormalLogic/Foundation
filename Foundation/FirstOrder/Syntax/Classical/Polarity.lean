...
@[simp] lemma polarity_rel {k} (r : L.Rel k) (v : Fin k → Semiterm L ξ n) :
    (rel r v).polarity = true := rfl

@[simp] lemma polarity_nrel {k} (r : L.Rel k) (v : Fin k → Semiterm L ξ n) :
    (nrel r v).polarity = false := rfl
...
@[simp] lemma polarity_and (φ ψ : Semiformula L ξ n) :
    (φ ⋏ ψ).polarity = (φ.polarity || ψ.polarity) := rfl

@[simp] lemma polarity_or (φ ψ : Semiformula L ξ n) :
    (φ ⋎ ψ).polarity = (φ.polarity && ψ.polarity) := rfl
...
@[simp] lemma polarity_imply {n} (φ ψ : Semiformula L ξ n) :
    (φ 🡒 ψ).polarity = (!φ.polarity && ψ.polarity) := by
  simp [imp_eq]
...
@[simp] lemma rel_positive {k} (r : L.Rel k) (v : Fin k → Semiterm L ξ n) : Positive (rel r v) :=
  rfl

@[simp] lemma rel_negative {k} (r : L.Rel k) (v : Fin k → Semiterm L ξ n) :
    ¬Negative (rel r v) := by simp [Negative]

@[simp] lemma nrel_positive {k} (r : L.Rel k) (v : Fin k → Semiterm L ξ n) :
    ¬Positive (nrel r v) := by simp [Positive]

@[simp] lemma nrel_negative {k} (r : L.Rel k) (v : Fin k → Semiterm L ξ n) : Negative (nrel r v) :=
  rfl
...
@[simp] lemma neg_positive_iff {n} (φ : Semiformula L ξ n) : (∼φ).Positive ↔ φ.Negative := by
  simp [Positive, Negative]

@[simp] lemma neg_negative_iff {n} (φ : Semiformula L ξ n) : (∼φ).Negative ↔ φ.Positive := by
  simp [Positive, Negative]
...
@[simp] lemma polarity_rew (ω : Rew L ξ₁ n₁ ξ₂ n₂) (φ : Semiformula L ξ₁ n₁) :
    (ω ▹ φ).polarity = φ.polarity := by
  induction φ using rec' <;> simp [polarity, *]
