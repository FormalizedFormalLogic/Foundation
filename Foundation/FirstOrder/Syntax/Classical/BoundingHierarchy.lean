...
lemma sigma_succ_induction {s : ℕ} {P : (n : ℕ) → Semiformula L ξ n → Prop}
    ...
    (hBall : ∀ R (_hR : R ∈ ℬ) n t φ, ℬ.Hierarchy 𝚺 (s + 1) φ → P (n + 1) φ → P n (∀¹[R.operator ![#0, Rew.bShift t]] φ))
    (hBexs : ∀ R (_hR : R ∈ ℬ) n t φ, ℬ.Hierarchy 𝚺 (s + 1) φ → P (n + 1) φ → P n (∃¹[R.operator ![#0, Rew.bShift t]] φ))
...
instance : LogicalConnective.AndOrClosed (ℬ.Hierarchy Γ s : Semiformula L ξ k → Prop) where
  verum := verum _ _ _
  falsum := falsum _ _ _
  and := and
  or := or

instance : LogicalConnective.Closed (ℬ.Hierarchy Γ 0 : Semiformula L ξ k → Prop) where
  not := by simp
