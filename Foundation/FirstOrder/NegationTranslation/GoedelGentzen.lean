...
theorem gödel_gentzen {T : Theory L} {φ} : T ⊢ φ → T.ToTheoryᵢ Λ ⊢ φᴺ := by
  ...
  have H : Λ ⊢ (↑s.conj : Proposition L)ᴺ ➝ ↑φᴺ :=
    by simpa [Semiformula.emb_doubleNegation] using (K!_right (Derivation.imply_doubleNegation _ _)) ⨀ this
  let U : Set (Propositionᵢ L) := (Rewriting.emb '' (T.ToTheoryᵢ Λ).theory)
  suffices
      U *⊢[Λ] ⋀ s.toList.map fun φ : Sentence L ↦ (↑φ : Proposition L)ᴺ by
