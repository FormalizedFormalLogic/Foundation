...
structure TIsDerivation (Γ : Sequent V L) where
  val : V
  derivationOf : IsDerivationOf L val Γ.val

attribute [simp] TIsDerivation.derivationOf

abbrev TDerivation (_T : InternalTheory V L) (Γ : Sequent V L) := TIsDerivation Γ

scoped infix:45 " ⊢!ᵈᵉʳ " => TDerivation

structure TIsProof (T : InternalTheory V L) (φ : Formula V L) where
  val : V
  proof : IsProof T.theory val φ.val

def TProof (T : InternalTheory V L) (φ : Formula V L) := TIsProof T φ
...
noncomputable def Derivable.toTDerivation (Γ : Sequent V L) (h : Derivable L Γ.val) : T ⊢!ᵈᵉʳ Γ := by
  exact ⟨Classical.choose h, Classical.choose_spec h⟩

lemma TDerivation.toDerivable {Γ : (Sequent V L)} (d : T ⊢!ᵈᵉʳ Γ) : Derivable L Γ.val :=
  ⟨d.val, d.derivationOf⟩

lemma TIsProof.toProvable {σ : Formula V L} (d : TIsProof T σ) : Provable T.theory σ.val :=
  ⟨d.val, d.proof⟩

noncomputable def Provable.toTIsProof {σ : Formula V L} (h : Provable T.theory σ.val) : TIsProof T σ := by
  exact ⟨Classical.choose h, Classical.choose_spec h⟩
...
lemma TProvable.iff_provable {σ : Formula V L} :
  ...
  constructor
  · intro b
    exact b.get.toProvable
  · intro h
    exact ⟨Provable.toTIsProof h⟩

alias ⟨toProvable, Provable.toTProvable⟩ := TProvable.iff_provable

noncomputable def proof_to_tDerivation {σ : Formula V L} (d : T ⊢!ᵈᵉʳ insert σ ∅) : T ⊢!ᵈᵉʳ insert σ ∅ := d
...
noncomputable def TDerivation.toTProof {φ} (d : T ⊢!ᵈᵉʳ insert φ ∅) : T ⊢! φ :=
  ⟨⟪∅, d.val⟫, by
    constructor
    · intro x hx; simpa using hx
    · simpa [IsProof] using d.derivationOf⟩

def TDerivation.of_eq (d : T ⊢!ᵈᵉʳ Γ) (e : Γ = Δ) : T ⊢!ᵈᵉʳ Δ := by rcases e; exact d

lemma TProof.toProvable {φ} (d : T ⊢! φ) : Provable T.theory φ.val := TIsProof.toProvable d
...
noncomputable def em (φ) (h : φ ∈ Γ := by simp) (hn : ∼φ ∈ Γ := by simp) : T ⊢!ᵈᵉʳ Γ :=
  ⟨identity Γ.val φ.val, by simp, IsDerivation.identity (by simp) h hn⟩

@[simp] lemma em_val (φ) (h : φ ∈ Γ) (hn : ∼φ ∈ Γ) :
    (em φ h hn : T ⊢!ᵈᵉʳ Γ).val = Bootstrapping.identity Γ.val φ.val := rfl

noncomputable def verum (h : ⊤ ∈ Γ := by simp) : T ⊢!ᵈᵉʳ Γ :=
  ⟨verumIntro Γ.val, by simp, IsDerivation.verumIntro (by simp) h⟩
...
noncomputable def and' (H : φ ⋏ ψ ∈ Γ) (dp : T ⊢!ᵈᵉʳ insert φ Γ) (dq : T ⊢!ᵈᵉʳ insert ψ Γ) : T ⊢!ᵈᵉʳ Γ :=
  ⟨andIntro Γ.val φ.val ψ.val dp.val dq.val, by simp,
    IsDerivation.andIntro (by simpa) (by simpa using dp.derivationOf) (by simpa using dq.derivationOf)⟩
...
noncomputable def or' (H : φ ⋎ ψ ∈ Γ) (dpq : T ⊢!ᵈᵉʳ insert φ (insert ψ Γ)) : T ⊢!ᵈᵉʳ Γ :=
  ⟨orIntro Γ.val φ.val ψ.val dpq.val, by simp, IsDerivation.orIntro (by simpa) (by simpa using dpq.derivationOf)⟩
...
noncomputable def all' {φ : Semiformula V L 1} (H : ∀⁰ φ ∈ Γ) (dp : T ⊢!ᵈᵉʳ insert φ.free Γ.shift) : T ⊢!ᵈᵉʳ Γ :=
  ⟨allIntro Γ.val φ.val dp.val, by simp, IsDerivation.allIntro (by simpa) (by simpa using dp.derivationOf)⟩
...
noncomputable def exs' {φ : Semiformula V L 1} (H : ∃⁰ φ ∈ Γ) (t : Term V L) (dp : T ⊢!ᵈᵉʳ insert (φ.subst ![t]) Γ) : T ⊢!ᵈᵉʳ Γ :=
  ⟨exsIntro Γ.val φ.val t.val dp.val, by simp, IsDerivation.exsIntro (by simpa) (by simp) (by simpa using dp.derivationOf)⟩
...
noncomputable def wk (d : T ⊢!ᵈᵉʳ Δ) (h : Δ ⊆ Γ) : T ⊢!ᵈᵉʳ Γ :=
  ⟨contraction Γ.val d.val, by simp, IsDerivation.contraction (by simp) h d.derivationOf⟩

@[simp] lemma wk_val (d : T ⊢!ᵈᵉʳ Δ) (h : Δ ⊆ Γ) : (wk d h).val = contraction Γ.val d.val := rfl

noncomputable def shift (d : T ⊢!ᵈᵉʳ Γ) : T ⊢!ᵈᵉʳ Γ.shift :=
  ⟨shiftRule Γ.shift.val d.val, by simp [Sequent.shift], IsDerivation.shiftRule (by simpa using d.derivationOf)⟩
...
noncomputable def cut (d₁ : T ⊢!ᵈᵉʳ insert φ Γ) (d₂ : T ⊢!ᵈᵉʳ insert (∼φ) Γ) : T ⊢!ᵈᵉʳ Γ :=
  ⟨cutRule Γ.val φ.val d₁.val d₂.val, by simp, IsDerivation.cutRule (by simpa using d₁.derivationOf) (by simpa using d₂.derivationOf)⟩
...
def ofSubset (h : T ⊆ U) (d : T ⊢!ᵈᵉʳ Γ) : U ⊢!ᵈᵉʳ Γ := d
...
noncomputable def byAxm {φ : Formula V L} (h : φ ∈' T.theory) : T ⊢! φ := by
  refine Provable.toTIsProof (T := T) (Derivable.toProvable (T := T.theory) ?_)
  refine ⟨{φ.val}, ?_, ?_⟩
  · intro x hx
    simpa using hx ▸ h
  · apply Derivable.em
...
noncomputable def ofSubset (h : T ⊆ U) {φ : Formula V L} (d : T ⊢! φ) : U ⊢! φ :=
  ⟨d.val, by
    rcases d.proof with ⟨hT, hd⟩
    exact ⟨fun x hx ↦ h (hT x hx), hd⟩⟩
...
noncomputable instance : Entailment.NegationEquiv T where
  negEquiv {φ} := by
    apply TDerivation.toTProof
    suffices T ⊢!ᵈᵉʳ insert ((φ ⋎ ∼φ ⋎ ⊥) ⋏ (φ ⋏ ⊤ ⋎ ∼φ)) ∅ by
...
noncomputable instance : Entailment.Minimal T where
  ...
  or₂ {φ ψ} := by
    apply TDerivation.toTProof
    suffices T ⊢!ᵈᵉʳ insert (∼ψ ⋎ φ ⋎ ψ) ∅ by
  ...
  or₃ {φ ψ r} := by
    apply TDerivation.toTProof
    suffices T ⊢!ᵈᵉʳ insert (φ ⋏ ∼r ⋎ ψ ⋏ ∼r ⋎ ∼φ ⋏ ∼ψ ⋎ r) ∅ by
...
noncomputable instance : Entailment.Cl T where
  dne {φ} := by
    apply TDerivation.toTProof
    simpa [Axioms.DNE, Semiformula.imp_def] using TDerivation.or (TDerivation.em φ)
