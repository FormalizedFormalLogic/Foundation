...
`Theory.DecidableΔ₁` is that interface. Note that `decide_iff` is stated over **raw** codes
`p : ℕ`, not only over `⌜σ⌝`: `Derivation.Phi`'s `axm` disjunct quantifies over arbitrary `p ∈ s`,
whereas `Δ₁Class.mem_iff` pins `Δ₁Class` down only at codes of sentences. A recognizer that decided
...
definition. For the combinators below it *is* sound on non-codes, and `evalb_ofList_ch` is where
that is proved.
...
/-- The headline instance: the axioms of `𝗣𝗔⁻` are executably recognizable. -/
...
Note the inputs are numerals. A set *literal* such as `{p, q} : ℕ` is itself built from
`Exp.exp` and does not reduce, so `decide` must be fed codes, not sets. -/
