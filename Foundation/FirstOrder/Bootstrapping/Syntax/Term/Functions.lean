...
@[simp] lemma lt_qqAdd_left (x y : V) : x < x ^+ y := by
  simpa [qqAdd, qqMul] using nth_lt_qqFunc_of_lt (i := 0) (k := 2) (f := (addIndex : V)) (v := ?[x, y]) (by simp)

@[simp] lemma lt_qqAdd_right (x y : V) : y < x ^+ y := by
  simpa [qqAdd, qqMul] using nth_lt_qqFunc_of_lt (i := 1) (k := 2) (f := (addIndex : V)) (v := ?[x, y]) (by simp)

@[simp] lemma lt_qqMul_left (x y : V) : x < x ^* y := by
  simpa [qqAdd, qqMul] using nth_lt_qqFunc_of_lt (i := 0) (k := 2) (f := (mulIndex : V)) (v := ?[x, y]) (by simp)

@[simp] lemma lt_qqMul_right (x y : V) : y < x ^* y := by
  simpa [qqAdd, qqMul] using nth_lt_qqFunc_of_lt (i := 1) (k := 2) (f := (mulIndex : V)) (v := ?[x, y]) (by simp)
