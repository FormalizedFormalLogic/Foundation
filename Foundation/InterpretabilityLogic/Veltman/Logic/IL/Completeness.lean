...
variable {Φ : AdequateSet α} {Γ Δ Θ : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 }}

structure ILSuccessor (Γ Δ : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 }) : Prop where
  prop1 : (∀ φ ∈ Γ.1.1.prebox, φ ∈ Δ.1 ∧ □φ ∈ Δ.1)
  prop2 : (∃ φ ∈ Δ.1.1.prebox, □φ ∉ Γ.1)
infix:80 " ≺ " => ILSuccessor

structure ILCriticalSuccessor (χ : { χ : Formula α // χ ∈ Φ.1}) (Γ Δ : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 }) extends Γ ≺ Δ where
...
lemma claim3 [Entailment.IL 𝓢] (h₁ : ∼(φ ▷ χ.1) ∈ Γ.1) : ∃ Δ, (Γ ≺[χ] Δ) ∧ φ ∈ Δ.1 := by
  ...
  have : □-φ ∉ Γ.1 := by
    by_contra hC;
    replace hC : Γ *⊢[𝓢] □-φ := ConsistentComplementClosedFinset.membership_iff (by simpa) |>.mp hC;
    apply ConsistentComplementClosedFinset.iff_mem_compl (by simpa) |>.mpr $ Formula.complement.rhd_def ▸ h₁;
    apply ConsistentComplementClosedFinset.membership_iff (by simpa) |>.mpr;
  ...
  have Δ_consis : Δ.Consistent 𝓢 := by
    by_contra!;
    ...
    . sorry;
    . sorry;
  obtain ⟨Ω, hΩ⟩ : ∃ Ω : ConsistentComplementClosedFinset 𝓢 Φ.1, Δ ⊆ Ω.1 := ConsistentComplementClosedFinset.lindenbaum Δ_consis;
  sorry;
  /-
  use ⟨Δ, by sorry⟩;
  constructor;
...
lemma claim4 (h₁ : (φ ▷ ψ) ∈ Γ.1) (h₂ : φ ∈ Δ.1) (h₃ : Γ ≺[χ] Δ)
  ...
  obtain ⟨Δ, hΔ⟩ : ∃ Δ : ConsistentComplementClosedFinset 𝓢 Φ.1, Δ₀ ⊆ Δ.1 := by
    sorry;
  use ⟨Δ, by sorry⟩;
  refine ⟨?_, ?_, ?_⟩;
  . exact {
...
variable {Φ : AdequateSet _} {𝓢 : S} {Γ : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 }}

protected inductive ILMiniCanonicalModel.IsWorld (Γ : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 })
  : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 } × List { φ // φ ∈ Φ.1 } → Prop
...
def ILMiniCanonicalModel (Γ : { Γ : ConsistentComplementClosedFinset 𝓢 Φ.1 // Γ.1 ⊆ Φ.1 }) : Veltman.Model where
  toKripkeFrame := {
...
instance IL.Veltman.finite_complete : Complete InterpretabilityLogic.IL Veltman.FrameClass.FiniteIL := by
  ...
  obtain ⟨Γ, hΓ⟩ : ∃ Γ : ConsistentComplementClosedFinset (InterpretabilityLogic.IL) Φ.1, {-φ} ⊆ Γ.1 := ConsistentComplementClosedFinset.lindenbaum (by sorry)
