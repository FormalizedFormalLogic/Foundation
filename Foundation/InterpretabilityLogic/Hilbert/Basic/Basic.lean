...
instance : Logic.Substitution (Hilbert.Basic Ax) where
  subst {φ} s h := by
    ...
    induction h with
    | @axm _ s' ih        => have h2 := Basic.axm (s := s' ∘ s) ih; rw [Formula.comp_subst] at h2; exact h2;
...
instance [Ax.HasJ1] : InterpretabilityLogic.Entailment.HasAxiomJ1 (Hilbert.Basic Ax) where
  axiomJ1! {φ ψ} := by
    constructor;
    have h := Hilbert.Basic.axm (Ax := Ax)
...
instance [Ax.HasJ2] : InterpretabilityLogic.Entailment.HasAxiomJ2 (Hilbert.Basic Ax) where
  axiomJ2! {φ ψ χ} := by
    ...
    have h := Hilbert.Basic.axm (Ax := Ax)
      ...
      $ HasJ2.mem_J2;
    simp [HasJ2.ne_pq, HasJ2.ne_qr, HasJ2.ne_rp.symm] at h;
    exact h
...
instance [Ax.HasJ3] : InterpretabilityLogic.Entailment.HasAxiomJ3 (Hilbert.Basic Ax) where
  axiomJ3! {φ ψ χ} := by
    ...
    have h := Hilbert.Basic.axm (Ax := Ax)
      ...
      $ HasJ3.mem_J3;
    simp [HasJ3.ne_pq, HasJ3.ne_qr, HasJ3.ne_rp.symm] at h;
    exact h
...
instance [Ax.HasM] : InterpretabilityLogic.Entailment.HasAxiomM (Hilbert.Basic Ax) where
  axiomM! {φ ψ χ} := by
    ...
    have h := Hilbert.Basic.axm (Ax := Ax)
      ...
      $ HasM.mem_M;
    simp [HasM.ne_pq, HasM.ne_qr, HasM.ne_rp.symm] at h;
    exact h
...
instance [Ax.HasWstar] : InterpretabilityLogic.Entailment.HasAxiomWstar (Hilbert.Basic Ax) where
  axiomWstar! {φ ψ χ} := by
    ...
    have h := Hilbert.Basic.axm (Ax := Ax)
      ...
      $ HasWstar.mem_Wstar;
    simp [HasWstar.ne_pq, HasWstar.ne_qr, HasWstar.ne_rp.symm] at h;
    exact h
...
instance [Ax.HasR] : InterpretabilityLogic.Entailment.HasAxiomR (Hilbert.Basic Ax) where
  axiomR! {φ ψ χ} := by
    ...
    have h := Hilbert.Basic.axm (Ax := Ax)
      ...
      $ HasR.mem_R;
    simp [HasR.ne_pq, HasR.ne_qr, HasR.ne_rp.symm] at h;
    exact h
...
instance [Ax.HasRstar] : InterpretabilityLogic.Entailment.HasAxiomRstar (Hilbert.Basic Ax) where
  axiomRstar! {φ ψ χ} := by
    ...
    have h := Hilbert.Basic.axm (Ax := Ax)
      ...
      $ HasRstar.mem_Rstar;
    simp [HasRstar.ne_pq, HasRstar.ne_qr, HasRstar.ne_rp.symm] at h;
    exact h
