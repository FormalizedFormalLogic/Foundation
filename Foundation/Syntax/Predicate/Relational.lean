...
variable {L : Language} [L.Relational] {ξ : Type*} {n : ℕ}
...
@[simp] lemma relationalVal_bvar (x : Fin n) :
    (#x : Semiterm L ξ n).relationalVal bv fv = bv x := rfl

@[simp] lemma relationalVal_fvar (x : ξ) : (&x : Semiterm L ξ n).relationalVal bv fv = fv x := rfl

lemma relationalVal_rew {ξ₁ ξ₂ : Type*} {n₁ n₂ : ℕ} {bv : Fin n₂ → M} {fv : ξ₂ → M}
    (ω : Rew L ξ₁ n₁ ξ₂ n₂) (t : Semiterm L ξ₁ n₁) :
    relationalVal bv fv (ω t) =
      relationalVal (relationalVal bv fv ∘ ω ∘ bvar) (relationalVal bv fv ∘ ω ∘ fvar) t := by
