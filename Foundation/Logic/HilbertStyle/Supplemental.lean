...
def efq_of_mem_either [DecidableEq F] [HasAxiomEFQ 𝓢] (h₁ : φ ∈ Γ) (h₂ : ∼φ ∈ Γ) : Γ ⊢[𝓢] ψ := of_O $ bot_of_mem_either h₁ h₂
...
def dn [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢ φ ⭤ ∼∼φ := E_intro dni dne
@[simp] lemma dn! [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢! φ ⭤ ∼∼φ := ⟨dn⟩
...
lemma CNNNN!_of_C! [DecidableEq F] (b : 𝓢 ⊢! φ ➝ ψ) : 𝓢 ⊢! ∼∼φ ➝ ∼∼ψ := ⟨CNNNN_of_C b.some⟩
...
def CN_of_CN_left [DecidableEq F] [HasAxiomDNE 𝓢] (b : 𝓢 ⊢ ∼φ ➝ ψ) : 𝓢 ⊢ ∼ψ ➝ φ := C_trans (contra b) dne
lemma CN!_of_CN!_left [DecidableEq F] [HasAxiomDNE 𝓢] (b : 𝓢 ⊢! ∼φ ➝ ψ) : 𝓢 ⊢! ∼ψ ➝ φ := ⟨CN_of_CN_left b.some⟩

def CCNCN' [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢ (∼φ ➝ ψ) ➝ (∼ψ ➝ φ) := deduct' $ CN_of_CN_left FiniteContext.id
@[simp] lemma CCNCN'! [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢! (∼φ ➝ ψ) ➝ (∼ψ ➝ φ) := ⟨CCNCN'⟩


def C_of_CNN [DecidableEq F] [HasAxiomDNE 𝓢] (b : 𝓢 ⊢ ∼φ ➝ ∼ψ) : 𝓢 ⊢ ψ ➝ φ := C_trans dni (CN_of_CN_left b)
lemma C!_of_CNN! [DecidableEq F] [HasAxiomDNE 𝓢] (b : 𝓢 ⊢! ∼φ ➝ ∼ψ) : 𝓢 ⊢! ψ ➝ φ := ⟨C_of_CNN b.some⟩

def CCNNC [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢ (∼φ ➝ ∼ψ) ➝ (ψ ➝ φ) :=  deduct' $ C_of_CNN FiniteContext.id
@[simp] lemma CCNNC! [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢! (∼φ ➝ ∼ψ) ➝ (ψ ➝ φ) := ⟨CCNNC⟩
...
lemma EN!_of_EN!_left [DecidableEq F] [HasAxiomDNE 𝓢] (h : 𝓢 ⊢! ∼φ ⭤ ψ) : 𝓢 ⊢! φ ⭤ ∼ψ := ⟨EN_of_EN_left h.some⟩
...
def ECCOO [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢ φ ⭤ ((φ ➝ ⊥) ➝ ⊥) := E_trans dn ENNCCOO
lemma ECCOO! [DecidableEq F] [HasAxiomDNE 𝓢] : 𝓢 ⊢! φ ⭤ ((φ ➝ ⊥) ➝ ⊥) := ⟨ECCOO⟩
...
def C_of_AN [DecidableEq F] [HasAxiomEFQ 𝓢] (b : 𝓢 ⊢ ∼φ ⋎ ψ) : 𝓢 ⊢ φ ➝ ψ := CANC ⨀ b
lemma C!_of_AN! [DecidableEq F] [HasAxiomEFQ 𝓢] (b : 𝓢 ⊢! ∼φ ⋎ ψ) : 𝓢 ⊢! φ ➝ ψ := ⟨C_of_AN b.some⟩
...
def left_Disj_intro [DecidableEq F] [HasAxiomEFQ 𝓢] (Γ : List F) (b : (ψ : F) → ψ ∈ Γ → 𝓢 ⊢ ψ ➝ φ) : 𝓢 ⊢ Γ.disj ➝ φ :=
  match Γ with
  |     [] => efq
  | ψ :: Γ => left_A_intro (b ψ (by simp)) <| left_Disj_intro Γ fun ψ h ↦ b ψ (by simp [h])
def left_Disj!_intro [DecidableEq F] [HasAxiomEFQ 𝓢] (Γ : List F) (b : (ψ : F) → ψ ∈ Γ → 𝓢 ⊢! ψ ➝ φ) : 𝓢 ⊢! Γ.disj ➝ φ :=
...
def left_Disj'_intro [DecidableEq F] [HasAxiomEFQ 𝓢] (l : List ι) (ψ : ι → F) (b : ∀ i ∈ l, 𝓢 ⊢ ψ i ➝ φ) : 𝓢 ⊢ l.disj' ψ ➝ φ :=
  left_Disj₂_intro _ fun χ h ↦
    let ⟨i, hi, e⟩ := l.chooseX (ψ · = χ) (by simpa using h)
    Entailment.cast (by simp [e]) (b i hi)
lemma left_Disj'!_intro [DecidableEq F] [HasAxiomEFQ 𝓢] (l : List ι) (ψ : ι → F) (b : ∀ i ∈ l, 𝓢 ⊢! ψ i ➝ φ) : 𝓢 ⊢! l.disj' ψ ➝ φ :=
...
lemma left_Fdisj'!_intro [DecidableEq F] [HasAxiomEFQ 𝓢] (s : Finset ι) (ψ : ι → F) (b : ∀ i ∈ s, 𝓢 ⊢! ψ i ➝ φ) : 𝓢 ⊢! (⩖ i ∈ s, ψ i) ➝ φ :=
  left_Disj'!_intro _ _ (by simpa)

lemma left_Udisj!_intro [DecidableEq F] [HasAxiomEFQ 𝓢] [Fintype ι] (ψ : ι → F) (b : (i : ι) → 𝓢 ⊢! ψ i ➝ φ) : 𝓢 ⊢! (⩖ i, ψ i) ➝ φ :=
...
def ofAOfN [DecidableEq F] (b : 𝓢 ⊢ φ ⋎ ψ) (d : 𝓢 ⊢ ∼φ) : 𝓢 ⊢ ψ := A_cases (C_of_CNN (dhyp d)) (C_id _) b

def of_a!_of_n! [DecidableEq F] (b : 𝓢 ⊢! φ ⋎ ψ) (d : 𝓢 ⊢! ∼φ) : 𝓢 ⊢! ψ := ⟨ofAOfN b.get d.get⟩
