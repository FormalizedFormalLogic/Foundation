...
def dne : 𝓢 ⊢ ∼∼p ➝ p := by
  have d₁ : 𝓢 ⊢ ∼∼p ➝ (∼∼(∼∼p) ➝ ∼∼p) ➝ ∼p ➝ ∼(∼∼p) := imply₁' $ elim_contra;
...
def explodeHyp (h₁ : 𝓢 ⊢ p ➝ q) (h₂ : 𝓢 ⊢ p ➝ ∼q) : 𝓢 ⊢ p ➝ r := by
  have : 𝓢 ⊢ p ➝ ∼q ➝ ∼r ➝ ∼q := imply₁' imply₁ (q := p)
  have : 𝓢 ⊢ p ➝ ∼r ➝ ∼q := this ⨀₁ h₂;
  have : 𝓢 ⊢ p ➝ q ➝ r := (imply₁' elim_contra (q := p)) ⨀₁ this;
  exact this ⨀₁ h₁;

def explodeHyp₂ (h₁ : 𝓢 ⊢ p ➝ q ➝ r) (h₂ : 𝓢 ⊢ p ➝ q ➝ ∼r) : 𝓢 ⊢ p ➝ q ➝ s := by
  have : 𝓢 ⊢ p ➝ q ➝ ∼r ➝ ∼s ➝ ∼r := imply₁' (imply₁' imply₁ (q := q)) (q := p)
  have : 𝓢 ⊢ p ➝ q ➝ ∼(s) ➝ ∼r := this ⨀₂ h₂;
  have : 𝓢 ⊢ p ➝ q ➝ r ➝ s := (imply₁' (imply₁' elim_contra (q := q)) (q := p)) ⨀₂ this;
  exact this ⨀₂ h₁;
...
def impTrans'₂ (bqr: 𝓢 ⊢ q ➝ r) : 𝓢 ⊢ (p ➝ q) ➝ (p ➝ r) := imply₂ ⨀ (imply₁' bqr)

def impTrans₂ : 𝓢 ⊢ (q ➝ r) ➝ (p ➝ q) ➝ (p ➝ r) := impTrans'' (impSwap (imply₁' (impId (q ➝ r)))) mdp₂In₁

def impTrans₁ : 𝓢 ⊢ (p ➝ q) ➝ (q ➝ r) ➝ (p ➝ r) := impSwap impTrans₂

def dhypBoth (h : 𝓢 ⊢ q ➝ r) : 𝓢 ⊢ (p ➝ q) ➝ (p ➝ r) := imply₂ ⨀ (imply₁' $ h)
...
def orElim : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (p ⋎ q ➝ r) := by
  ...
  have d₁ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ (p ➝ r) ➝ ∼r ➝ ∼p
    := (imply₁' (q := p ➝ r) <| imply₁' (q := q ➝ r) <| imply₁' (q := ∼p ➝ q) <| contraIntro (p := p) (q := r));
  have d₂ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ ∼r ➝ ∼p
    := d₁ ⨀₃ (imply₁₁ (p ➝ r) (q ➝ r) (∼p ➝ q));
  have d₃ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ ∼r ➝ q
    := (imply₁' (q := p ➝ r) <| imply₁' (q := q ➝ r) <| imply₁ (p := ∼p ➝ q) (q := ∼r)) ⨀₄ d₂;
  have d₄ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ ∼r ➝ r
    := (imply₁' (q := p ➝ r) <| imply₁₁ (p := q ➝ r) (q := ∼p ➝ q) (r := ∼r)) ⨀₄ d₃;
  have d₅ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ ∼r ➝ r ➝ ⊥
    := by simpa using imply₁' (q := p ➝ r) <| imply₁' (q := q ➝ r) <| imply₁' (q := ∼p ➝ q) <| impId (p := ∼r);
  have d₆ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ ∼∼r
    := by simpa using d₅ ⨀₄ d₄;
  have d₇ : 𝓢 ⊢ (p ➝ r) ➝ (q ➝ r) ➝ (∼p ➝ q) ➝ ∼∼r ➝ r
    := imply₁' (q := p ➝ r) <| imply₁' (q := q ➝ r) <| imply₁' (q := ∼p ➝ q) <| dne (p := r);
  exact d₇ ⨀₃ d₆;
