...
@[simp] lemma models_list_conj' {l : List α} {ι : α → F} :
    𝓜 ⊧ l.conj' ι ↔ ∀ i ∈ l, 𝓜 ⊧ ι i := by simp [List.conj']

@[simp] lemma models_finset_conj {s : Finset F} :
    𝓜 ⊧ s.conj ↔ ∀ φ ∈ s, 𝓜 ⊧ φ := by simp [Finset.conj]

@[simp] lemma models_finset_conj' {s : Finset α} {ι : α → F} :
    𝓜 ⊧ s.conj' ι ↔ ∀ i ∈ s, 𝓜 ⊧ ι i := by simp [Finset.conj']
...
@[simp] lemma models_list_disj' {l : List α} {ι : α → F} :
    𝓜 ⊧ l.disj' ι ↔ ∃ i ∈ l, 𝓜 ⊧ ι i := by simp [List.disj']

@[simp] lemma models_finset_disj {s : Finset F} :
    𝓜 ⊧ s.disj ↔ ∃ φ ∈ s, 𝓜 ⊧ φ := by simp [Finset.disj]

@[simp] lemma models_finset_disj' {s : Finset α} {ι : α → F} :
    𝓜 ⊧ s.disj' ι ↔ ∃ i ∈ s, 𝓜 ⊧ ι i := by simp [Finset.disj']
...
lemma not_satisfiable_finset [LogicalConnective F] [LogicalNeutral F] [Tarski M] [DecidableEq F]
    (t : Finset F) :
    ¬Satisfiable M (t : Set F) ↔ Valid M (t.image (∼·)).disj := by
  simp [Satisfiable, modelsSet_iff, Valid];

@[simp] lemma satisfiable_conj₂ [LogicalConnective F] [LogicalNeutral F] [Tarski M]
    (l : List F) :
    Satisfiable M {⋀l} ↔ Satisfiable M {φ | φ ∈ l} := by
  simp [Satisfiable, modelsSet_iff]

@[simp] lemma satisfiable_fconj [LogicalConnective F] [LogicalNeutral F] [Tarski M]
    (s : Finset F) :
...
lemma models {T : Set F} {φ : F} (𝓜 : M) [𝓜 ⊧* T] (hf : φ ∈ T) : 𝓜 ⊧ φ :=
  models_set hf
...
lemma valid_neg_iff [LogicalConnective F] [LogicalNeutral F] [Tarski M] (φ : F) :
    Valid M (∼φ) ↔ ¬Satisfiable M {φ} := by
...
lemma set_models_iff {s : Set M} {φ : F} : s ⊧ φ ↔ ∀ 𝓜 ∈ s, 𝓜 ⊧ φ := iff_of_eq rfl

instance [LogicalNeutral F] [Semantics.Top M] : Semantics.Top (Set M) :=
  ⟨fun s ↦ by simp [set_models_iff]⟩

lemma set_meaningful_iff_nonempty [∀ 𝓜 : M, Meaningful 𝓜] {s : Set M} :
    Meaningful s ↔ s.Nonempty := by
  constructor;
  · rintro ⟨φ, hf⟩;
    by_contra A;
...
lemma meaningful_iff_satisfiableSet [∀ 𝓜 : M, Meaningful 𝓜] {T : Set F} :
...
lemma consequence_iff_not_satisfiable [LogicalConnective F] [LogicalNeutral F] [Tarski M]
    {T : Set F} {φ : F} :
...
/-- A `Semantics M F` is compact if, for any set of formulas, the satisfiability of the set
is equivalent to the satisfiability of every finite subset of it. -/
...
lemma conseq_compact [LogicalConnective F] [LogicalNeutral F] [Semantics.Tarski M]
    {T : Set F} {φ : F} :
