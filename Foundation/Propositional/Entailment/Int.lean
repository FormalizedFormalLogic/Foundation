...
@[grind ⇒] lemma of_O [ModusPonens 𝓢] [Entailment.HasAxiomEFQ 𝓢] (h : 𝓢 ⊢ ⊥) : 𝓢 ⊢ φ :=
  ⟨of_O! h.some⟩
  ...
  exact C!_trans
    (by
      ...
      have d₁ : [(∼∼φ 🡒 ∼∼ψ) ⋏ ∼(φ 🡒 ψ)] ⊢[𝓢]! ∼∼φ 🡒 ∼∼ψ :=
        K!_left (ψ := ∼(φ 🡒 ψ)) <| FiniteContext.id!;
      have d₂ : [(∼∼φ 🡒 ∼∼ψ) ⋏ ∼(φ 🡒 ψ)] ⊢[𝓢]! ∼∼φ ⋏ ∼ψ :=
        KNN!_of_NA! <| (contra! CANC!) ⨀ (K!_right (φ := (∼∼φ 🡒 ∼∼ψ)) <| FiniteContext.id!)
...
lemma CFDisjFDisj_of_subset {Γ Δ : Finset F} (h : Γ ⊆ Δ) : 𝓢 ⊢ Γ.disj 🡒 Δ.disj := by
  refine C_trans (C_trans ?_ (CDisj₂Disj₂_of_subset (Γ := Γ.toList) (Δ := Δ.toList)
    (by simpa))) ?_ <;> simp;
