...
instance instFrameClassSound (hV : C ⊧* Ax) : Sound (Hilbert.F Ax) C := ⟨fun {_} => soundness_frameclass hV⟩
...
instance instModelClassSound (hV : C ⊧* Ax) : Sound (Hilbert.F Ax) C := ⟨fun {_} => soundness_modelclass hV⟩
