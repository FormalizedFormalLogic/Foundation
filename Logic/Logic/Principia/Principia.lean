...
class Principia (F : Type u) [LogicalConnective F] (α : outParam (Type*)) extends Axiom F α where
  Pr : α → List F → F → Type u
  byAxiom (a : α) : σ ∈ @Axiom.toTheory F _ _ a → Pr a Δ σ

class LawfulPrincipia (F : Type u) [LogicalConnective F] [System F] (α : outParam (Type*)) [Principia F α] where
...
class Intuitionistic (F : Type u) [LogicalConnective F] (α : outParam (Type*)) extends Principia F α where
