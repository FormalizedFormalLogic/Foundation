...
@[simp]
def Formula.ExtendedKripkeValid (P : ∀ {α}, Kripke.Frame α → Prop) (p : Formula β) : Prop := ∀ {α}, ∀ (M : Kripke.Model α β), (M ⊧ᴾ[P] p)
notation "⊧ᴾ[" P "] " p => Formula.ExtendedKripkeValid P p

@[simp] lemma Formula.ExtendedKripkeValid.iff_int {p : Formula β} : (⊧ᴾ[(λ _ => True)] p) ↔ (⊧ⁱ p) := by
  simp [KripkeValid];
  rfl;

@[simp]
def Formula.ExtendedKripkeConsequence (P : ∀ {α : Type u}, Kripke.Frame α → Prop) (Γ : Theory β) (p : Formula β) : Prop := ∀ {α : Type u}, ∀ (M : Kripke.Model α β) w, (w ⊩ᴾ[P, M] Γ) → (w ⊩ᴾ[P, M] p)
notation:50 Γ " ⊨ᴾ[" P "] " p => Formula.ExtendedKripkeConsequence P Γ p

@[simp] lemma Formula.ExtendedKripkeConsequence.iff_int : (Γ ⊨ᴾ[(λ _ => True)] p) ↔ (Γ ⊨ⁱ p) := by simp; rfl;
