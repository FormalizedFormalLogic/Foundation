...
inductive DTerm (L : Q(Language.{u})) : ℕ → Type
  ...
  | finitary {n arity : ℕ} (f : Q(Subterm.Finitary.{u, 0} $L $arity)) : (Fin arity → DTerm L n) → DTerm L n
...
def rewFinitary {arity n m : ℕ} (ω : Q(Rew $L ℕ $n ℕ $m))
  (o : Q(Subterm.Finitary.{u, 0} $L $arity)) {v : Fin arity → DTerm L n} {w : Fin arity → DTerm L m}
  (H : ∀ i, v i ≡[qrew L n m ω] w i) : finitary o v ≡[qrew L n m ω] finitary o w := by
  let v' := Qq.vecFold q(SyntacticSubterm $L $n) (fun i => (v i).toExpr)
  let w' := Qq.vecFold q(SyntacticSubterm $L $m) (fun i => (w i).toExpr)
  let H : Q(∀ i, $ω ($v' i) = $w' i) := vecFoldDep q(fun i => $ω ($v' i) = $w' i) (fun i => (H i).expr)
  let e : Q($ω (($o).operator $v') = ($o).operator $w') := q(lemmataTerm.rew_finitary_eq_of_eq _ $o $H)
  exact .mk e

def rewConst {n m : ℕ} (ω : Q(Rew $L ℕ $n ℕ $m))
  (c : Q(Subterm.Const.{u, 0} $L)) : (const c : DTerm L n) ≡[qrew L n m ω] const c := by
